...
class AttemptTests(unittest.TestCase):
    def test_prelaunch_drift_blocks_execution(self):
        with tempfile.TemporaryDirectory() as tmp:
            p = Path(tmp) / "source"; p.write_bytes(b"before")
            expected = {str(p): run.prepare.sha(p)}
            p.write_bytes(b"after")
            with patch.object(run, "invoke") as invoke:
                with self.assertRaises(ValueError):
                    run.checked_invoke([], Path(tmp) / "attempt", "out", (expected,), {})
                invoke.assert_not_called()

    def test_wrong_frozen_copy_blocks_execution(self):
        with tempfile.TemporaryDirectory() as tmp:
            source = Path(tmp) / "source"; source.write_bytes(b"audited")
            copied = Path(tmp) / "copy"; copied.write_bytes(b"replaced")
            with patch.object(run, "invoke") as invoke:
                with self.assertRaises(ValueError):
                    run.checked_invoke([], Path(tmp) / "attempt", "out", (), {str(copied): run.prepare.sha(source)})
                invoke.assert_not_called()

    def test_unchanged_launch_proceeds_once(self):
        with tempfile.TemporaryDirectory() as tmp:
            p = Path(tmp) / "source"; p.write_bytes(b"audited")
            group = {str(p): run.prepare.sha(p)}
            with patch.object(run, "invoke", return_value={"exit_code": 0}) as invoke:
                self.assertEqual(run.checked_invoke([], Path(tmp) / "attempt", "out", (group,), group), {"exit_code": 0})
