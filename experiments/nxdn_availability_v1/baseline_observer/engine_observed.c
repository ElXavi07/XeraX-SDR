...
open_recording_outputs_if_needed(dsd_opts* opts, dsd_state* state) {
    ...
    if (opts->dmr_stereo_wav == 1 && !opts->analog_only && opts->wav_out_f == NULL && opts->wav_out_fR == NULL) {
        dsd_stat_t st;
...
dsd_engine_cleanup(dsd_opts* opts, dsd_state* state) {
    ...
    if (opts->iq_replay_requested) {
        LOG_INFO("XeraX P25 FEC: accepted=%u rejected=%u; voice_accepted=%u voice_rejected=%u\n",
            state->p25_p1_fec_ok + state->p25_p2_rs_facch_ok + state->p25_p2_rs_sacch_ok,
            state->p25_p1_fec_err + state->p25_p2_rs_facch_err + state->p25_p2_rs_sacch_err,
            state->p25_p1_voice_fec_ok, state->p25_p1_voice_fec_err);
    }
...
dsd_engine_run_with_lifecycle(dsd_opts* opts, dsd_state* state, const dsd_engine_lifecycle_hooks* hooks) {
    ...
    dsd_dmr_evidence_reset();
    dsd_voice_tune_result(0,0);
    dsd_equalizer_reset();
