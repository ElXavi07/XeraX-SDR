...
$taskOutput=Join-Path $taskRepo 'build/site-audio-check.jar'
& $taskJava -cp ($taskJars -join ';') org.jetbrains.kotlin.cli.jvm.K2JVMCompiler -no-stdlib -no-reflect -classpath $taskStdlib `
    (Join-Path $taskRepo 'upstream/dsd-neo/android/package/src/io/github/arancormonk/dsdneo/SiteAudioSelector.kt') `
    (Join-Path $taskRepo 'checks/SiteAudioSelectorCheck.kt') -d $taskOutput
if($LASTEXITCODE -ne 0) { throw 'Site audio compilation failed' }
& $taskJava -cp "$taskOutput;$taskStdlib" SiteAudioSelectorCheckKt
if($LASTEXITCODE -ne 0) { throw 'Site audio regression failed' }
