"""Verify the two 4.1.3 packages against corresponding source and fresh evidence."""
...
assert release['version']=='4.1.3' and release['versionCode']==40103
def digest(path): return hashlib.sha256(path.read_bytes()).hexdigest()
def log(name): return (root/'build'/name).read_text(encoding='utf-8-sig')
assert '100% tests passed out of 30' in log('receiver-413-checks.log')
assert '69 passed, 0 failed, 0 skipped' in log('receiver-413-qml.txt')
assert re.findall(r'100% tests passed out of (\d+)',log('receiver-413-full-iq.log'))==['30','1']
assert '100% tests passed out of 3' in log('receiver-413-ppm.log')
assert '100% tests passed out of 2' in log('receiver-413-core.log')
...
archive=dist/'XeraX-SDR-4.1.3-source.zip'
...
network=json.loads((root/'build/tcp-413-failures/report.json').read_text())
assert len(network)==4 and all(t['passed'] for t in network)
delayed=json.loads((root/'build/tcp-413-delayed-audio/report.json').read_text())
...
for suffix,abi in [('arm64','arm64-v8a'),('armeabi-v7a','armeabi-v7a')]:
    ...
    assert "name='com.xerax.sdr'" in metadata and "versionCode='40103'" in metadata
    assert "versionName='4.1.3'" in metadata and "minSdkVersion:'29'" in metadata
...
report=dict(networkFailureCases=network,delayedHeaderAudio=delayed,pcPhysicalAnalogProbe=physical,phoneAudioVerified=False,version='4.1.3',versionCode=40103,packages=packages,hostTestGroups=30,qmlCases=69,
...
(dist/'XeraX-SDR-4.1.3-verification.json').write_text(json.dumps(report,indent=2)+'\n',encoding='utf-8')
