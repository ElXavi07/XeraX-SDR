#include "../src/io/radio/hackrf_source.h"
...
Java_io_github_arancormonk_dsdneo_DsdNative_nativeSetAudioOutputDevice(JNIEnv*, jobject, jint device_id) {
    return static_cast<jint>(dsd_audio_set_output_device_id(static_cast<int>(device_id)));
}

extern "C" JNIEXPORT jint JNICALL
Java_io_github_arancormonk_dsdneo_DsdNative_nativeReceiverAction(JNIEnv*, jobject, jint action) {
    switch (action) {
        case 0: return dsd_app_command_action(DSD_APP_CMD_SCAN_HOLD_TOGGLE);
        case 1: return dsd_app_command_action(DSD_APP_CMD_CHANNEL_CYCLE);
        case 2: return dsd_app_command_action(DSD_APP_CMD_TOGGLE_MUTE);
        default: return -1;
    }
}
...
Java_io_github_arancormonk_dsdneo_DsdNative_nativeInit(JNIEnv* env, jclass clazz, jstring config_dir,
    ...
    set_env_from_jstring(env, "XERAX_PROCESS_DATA_DIR", config_dir);
...
Java_io_github_arancormonk_dsdneo_DsdNative_nativeCallIdentities(JNIEnv* env, jobject) {
    dsd_app_notification_status status{};
    if (!dsd_app_notification_get(&status)) return nullptr;
    jlong values[6]{};
    for (int i=0;i<2;i++) { values[i*3]=status.slots[i].state; values[i*3+1]=status.slots[i].tg_id; values[i*3+2]=status.slots[i].src_id; }
    auto result=env->NewLongArray(6); if(result) env->SetLongArrayRegion(result,0,6,values); return result;
}

extern "C" JNIEXPORT void JNICALL
Java_io_github_arancormonk_dsdneo_DsdNative_nativeSuppressLiveAudio(JNIEnv*, jobject, jboolean enabled) {
    dsd_audio_live_suppress(enabled ? 1 : 0);
}

extern "C" JNIEXPORT void JNICALL
Java_io_github_arancormonk_dsdneo_DsdNative_nativeClearWorkerEvidence(JNIEnv*, jobject) {
    dsd_qt::DiagnosticsLog::instance().clear();
    dsd_app_notification_status status{}; dsd_app_notification_get(&status);
}
extern "C" JNIEXPORT jstring JNICALL
Java_io_github_arancormonk_dsdneo_DsdNative_nativeWorkerEvidence(JNIEnv* env, jobject) {
    // Same bounded, redacted log that is visible in the diagnostics screen.
    dsd_dmr_evidence dmr{}; dsd_dmr_evidence_get(&dmr);
    const auto summary=QString("\nXeraX DMR evidence: checked=%1 suspected_ras=%2 rejected=%3")
        .arg(qulonglong(dmr.checked[0]+dmr.checked[1])).arg(qulonglong(dmr.suspected_ras[0]+dmr.suspected_ras[1])).arg(qulonglong(dmr.rejected[0]+dmr.rejected[1]));
    const auto text=(dsd_qt::DiagnosticsLog::instance().snapshot().join('\n').right(60000)+summary).toUtf8();
    return env->NewStringUTF(text.constData());
}
extern "C" JNIEXPORT void JNICALL
Java_io_github_arancormonk_dsdneo_DsdNative_nativeEqualizer(JNIEnv*, jobject, jboolean enabled) { dsd_equalizer_enable(enabled); }
extern "C" JNIEXPORT void JNICALL
Java_io_github_arancormonk_dsdneo_DsdNative_nativeRasReception(JNIEnv*, jobject, jboolean enabled) { dsd_dmr_ras_enable(enabled); }
extern "C" JNIEXPORT void JNICALL
Java_io_github_arancormonk_dsdneo_DsdNative_nativeDual(JNIEnv*, jobject, jboolean enabled) { dsd_dual_enable(enabled); }
extern "C" JNIEXPORT jlongArray JNICALL
Java_io_github_arancormonk_dsdneo_DsdNative_nativeDualGrant(JNIEnv* env, jobject) {
    dsd_dual_grant g{}; if(!dsd_dual_get(&g)) return nullptr;
    jlong values[]={static_cast<jlong>(g.sequence),g.frequency,g.wacn,g.sysid,g.nac,g.talkgroup,g.source,g.tdma,g.slot,g.priority};
    auto result=env->NewLongArray(10); if(result) env->SetLongArrayRegion(result,0,10,values); return result;
}
extern "C" JNIEXPORT void JNICALL
Java_io_github_arancormonk_dsdneo_DsdNative_nativeTrial(JNIEnv*, jobject, jdouble offset, jint bandwidth) { dsd_trial_configure(offset,bandwidth); }
extern "C" JNIEXPORT void JNICALL
Java_io_github_arancormonk_dsdneo_DsdNative_nativeSharedRate(JNIEnv*, jobject, jint rate) { dsd_channel_worker_rate_set(static_cast<uint32_t>(rate)); }
extern "C" JNIEXPORT jboolean JNICALL
Java_io_github_arancormonk_dsdneo_DsdNative_nativeVoiceTune(JNIEnv* env, jobject, jlongArray values, jint modulation) {
    if(!values || env->GetArrayLength(values)!=10) return JNI_FALSE;
    jlong v[10]; env->GetLongArrayRegion(values,0,10,v);
    for(int i=0;i<10;++i) if(v[i]<0 || (i>0 && v[i]>UINT32_MAX)) return JNI_FALSE;
    dsd_voice_tune t{};
    t.grant={static_cast<uint64_t>(v[0]),static_cast<uint32_t>(v[1]),static_cast<uint32_t>(v[2]),static_cast<uint32_t>(v[3]),
        static_cast<uint32_t>(v[4]),static_cast<uint32_t>(v[5]),static_cast<uint32_t>(v[6]),static_cast<int>(v[7]),static_cast<int>(v[8]),static_cast<int>(v[9])};
    t.modulation=modulation;
    if(!dsd_voice_tune_valid(&t)) return JNI_FALSE;
    dsd_voice_tune_result(t.grant.sequence,0);
    if(dsd_app_command_submit(DSD_APP_CMD_P25_VOICE_TUNE,&t,sizeof(t))<0) { dsd_voice_tune_result(t.grant.sequence,-1); return JNI_FALSE; }
    return JNI_TRUE;
}
extern "C" JNIEXPORT jlongArray JNICALL
Java_io_github_arancormonk_dsdneo_DsdNative_nativeVoiceTuneStatus(JNIEnv* env, jobject) {
    uint64_t sequence=0; const int status=dsd_voice_tune_status(&sequence);
    jlong values[]={static_cast<jlong>(sequence),status};
    auto out=env->NewLongArray(2); if(out) env->SetLongArrayRegion(out,0,2,values); return out;
}
extern "C" JNIEXPORT void JNICALL
Java_io_github_arancormonk_dsdneo_DsdNative_nativeCloseChannels(JNIEnv*, jobject) { dsd_channel_reset_source(); }

extern "C" JNIEXPORT void JNICALL
Java_io_github_arancormonk_dsdneo_DsdNative_nativeSiteCapture(JNIEnv*, jobject, jboolean enabled) {
    // Called on the service's configuration path, with no running engine.
    setenv("DSD_NEO_DISABLE_FS4_SHIFT", enabled ? "1" : "0", 1);
    dsd_channel_reset_source();
}
extern "C" JNIEXPORT jlongArray JNICALL
Java_io_github_arancormonk_dsdneo_DsdNative_nativeAudioProgress(JNIEnv* env, jobject) {
    dsd_app_notification_status status{}; dsd_app_notification_get(&status);
    jlong v[]={static_cast<jlong>(dsd_audio_received_frames()), static_cast<jlong>(dsd_audio_nonzero_frames()),
        static_cast<jlong>(dsd_audio_output_frames()),status.audio_muted};
    auto out=env->NewLongArray(4); if(out) env->SetLongArrayRegion(out,0,4,v); return out;
}

extern "C" JNIEXPORT jint JNICALL
Java_io_github_arancormonk_dsdneo_DsdNative_nativeTestAudioOutput(JNIEnv*, jobject, jint token) {
    return dsd_audio_test_output(token);
}
extern "C" JNIEXPORT void JNICALL
Java_io_github_arancormonk_dsdneo_DsdNative_nativeCancelAudioTest(JNIEnv*, jobject) {
    dsd_audio_cancel_output_test();
}

extern "C" JNIEXPORT jint JNICALL
Java_io_github_arancormonk_dsdneo_DsdNative_nativeAudioTestToken(JNIEnv*, jobject) { return dsd_audio_output_test_token(); }

extern "C" JNIEXPORT jint JNICALL
Java_io_github_arancormonk_dsdneo_DsdNative_nativeSetHackrfUsbFd(JNIEnv*, jobject, jint fd) {
    return xerax_hackrf_set_fd(static_cast<int>(fd));
}
