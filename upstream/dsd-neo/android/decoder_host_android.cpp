...
constexpr const char* kAudioRouterClass = "io/github/arancormonk/dsdneo/AudioOutputRouter";
...
DecoderHostAndroid::refreshPresentation() {
    ...
    const auto routingJson = QJniObject::callStaticObjectMethod(
                                 kAudioRouterClass, "snapshot", "(Landroid/content/Context;)Ljava/lang/String;",
                                 context.object()).toString();
    auto routing = QJsonDocument::fromJson(routingJson.toUtf8()).object().toVariantMap();
    const int actualDevice = dsd_audio_output_device_id();
    routing.insert(QStringLiteral("actualId"), actualDevice);
    const auto route = QJniObject::callStaticObjectMethod(
                           ...
                           context.object(), static_cast<jint>(actualDevice))
                           .toString();
    if (route != m_audio_route || routing != m_audio_output) {
        m_audio_route = route;
        m_audio_output = routing;
...
bool
DecoderHostAndroid::selectAudioOutput(const QString& key) {
    const auto context = android_context();
    if (!context.isValid()) {
        return false;
    }
    const bool accepted = QJniObject::callStaticMethod<jboolean>(
        kAudioRouterClass, "select", "(Landroid/content/Context;Ljava/lang/String;)Z",
