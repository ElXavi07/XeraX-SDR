...
object DsdNative {
    ...
    init {
        // Match the running process, including a 32-bit APK on a dual-ABI device.
        val abi = if (android.os.Process.is64Bit()) "arm64-v8a" else "armeabi-v7a"
        System.loadLibrary("dsd-neo-app_$abi")
    }
    ...
    external fun nativeSetAudioOutputDevice(deviceId: Int): Int
    external fun nativeCallIdentities(): LongArray?
    external fun nativeSuppressLiveAudio(enabled: Boolean)
    external fun nativeTestAudioOutput(token: Int): Int
    external fun nativeAudioTestToken(): Int
    external fun nativeCancelAudioTest()
    external fun nativeReceiverAction(action: Int): Int
    external fun nativeClearWorkerEvidence()
    external fun nativeWorkerEvidence(): String
    external fun nativeEqualizer(enabled: Boolean)
    external fun nativeDual(enabled: Boolean)
    external fun nativeDualGrant(): LongArray?
    external fun nativeTrial(offsetHz: Double, bandwidthHz: Int)
    external fun nativeSiteCapture(enabled: Boolean)
    external fun nativeAudioProgress(): LongArray
    external fun nativeSharedRate(rate: Int)
    external fun nativeVoiceTune(grant: LongArray, modulation: Int): Boolean
    external fun nativeVoiceTuneStatus(): LongArray
    external fun nativeCloseChannels()
    ...
    external fun nativeSetHackrfUsbFd(fd: Int): Int
    ...
    external fun nativeRasReception(enabled:Boolean)
}
