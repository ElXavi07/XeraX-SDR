...
class DecoderService : Service() {
    ...
    override fun onCreate() {
        ...
        AudioOutputRouter.initialize(this)

        ReceiverExtras.initialize(this)
        ReceiverSession.service=this
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        when (intent?.action) {
            ...
            ACTION_HOLD -> DsdNative.nativeReceiverAction(0)

            ACTION_NEXT -> DsdNative.nativeReceiverAction(1)

            ACTION_MUTE -> DsdNative.nativeReceiverAction(2)

            ACTION_SPEAKER -> AudioOutputRouter.select(this, "speaker")

            ACTION_START -> {

                ReceiverExtras.resumeLive(this)
    ...
    override fun onDestroy() {
        ...
        if(ReceiverSession.service===this) ReceiverSession.service=null
        super.onDestroy()
    ...
    fun resumeFromService(args:Array<String>) {
        if(synchronized(lock) { state }!=State.IDLE) return
        startDecoding(args)
        if(synchronized(lock) { state }==State.RUNNING) startStatusPolling()
    }
    fun finishRecoveryWait() { stopIfIdle() }
    ...
    private fun startDecoding(args: Array<String>) {
        ...
        ReceiverSession.begin(args)
        val siteCapture = args.contains("--xerax-site-capture")
        DsdNative.nativeSiteCapture(siteCapture)
        ReceiverWorkers.prepareSite(siteCapture)
        val configureRc = DsdNative.nativeConfigure(args.filter { it != "--xerax-site-capture" }.toTypedArray(), synchronized(lock) { sessionId })
        ...
        val thread = Thread({
            ...
            ReceiverWorkers.stopAll()
            DsdNative.nativeCloseChannels()
            ...
            val restore = ReceiverSession.completed(rc==DsdNative.STATUS_OK)
            if(restore!=null) statusHandler.post { resumeFromService(restore) }
            else if(!ReceiverSession.awaitingUsb()) stopSelfLatest()
    ...
    private fun buildNotification(text: String): Notification {
        ...
        val builder = Notification.Builder(this, CHANNEL_ID)
            ...
            .addAction(receiverAction("Hold / resume", ACTION_HOLD, 31))

            .addAction(receiverAction(ReceiverExtras.text("Next", "Siguiente"), ACTION_NEXT, 32))

            .addAction(receiverAction("Mute / unmute", ACTION_MUTE, 33))

            .addAction(receiverAction("Phone speaker", ACTION_SPEAKER, 34))

            .addAction(Notification.Action.Builder(android.graphics.drawable.Icon.createWithResource(this, android.R.drawable.ic_media_pause), getString(R.string.action_stop), stopPendingIntent).build())
        ...
        if (expanded.isNotEmpty() && expanded != body) {
            ...
            builder.setSubText(expanded.replace("\n", " · "))
    ...
    private fun receiverAction(label: String, action: String, request: Int): Notification.Action {

        val pending = PendingIntent.getService(this, request,

            Intent(this, DecoderService::class.java).setAction(action),

            PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)

        return Notification.Action.Builder(android.graphics.drawable.Icon.createWithResource(this, when (action) {

            ACTION_NEXT -> android.R.drawable.ic_media_next

            ACTION_MUTE -> android.R.drawable.ic_lock_silent_mode

            ACTION_SPEAKER -> android.R.drawable.ic_lock_silent_mode_off
    ...
    private val statusPoll = object : Runnable {
        ...
        override fun run() {
            ...
            if (record != lastStatusRecord) {
                ...
                ReceiverExtras.observe(this@DecoderService, lastStatus)
    ...
    companion object {
        @JvmStatic fun audioSessionActive(): Boolean = synchronized(lock) { state != State.IDLE }
        ...
        const val ACTION_HOLD = "com.xerax.sdr.HOLD"

        const val ACTION_NEXT = "com.xerax.sdr.NEXT"

        const val ACTION_MUTE = "com.xerax.sdr.MUTE"

        const val ACTION_SPEAKER = "com.xerax.sdr.SPEAKER"
