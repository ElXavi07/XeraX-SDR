...
if(PKG_CONFIG_FOUND)
    ...
    pkg_check_modules(
        PC_PULSEAUDIO_MAINLOOP
        libpulse-mainloop-glib>=11.0.0
        QUIET
    )
...
find_path(
    ...
    NAMES pulse/pulseaudio.h pulse/simple.h
...
elseif(
    PULSEAUDIO_INCLUDE_DIR
    AND EXISTS "${PULSEAUDIO_INCLUDE_DIR}/pulse/version.h"
)
    file(
        STRINGS "${PULSEAUDIO_INCLUDE_DIR}/pulse/version.h"
        pulseaudio_version_str
        REGEX "^#define[\t ]+pa_get_headers_version\\(\\)[\t ]+\\(\".*\"\\).*"
    )
    string(
        REGEX REPLACE
            "^#define[\t ]+pa_get_headers_version\\(\\)[\t ]+\\(\"([^\"]+)\"\\).*"
        "\\1"
...
if(PULSEAUDIO_FOUND)
    ...
    if(NOT TARGET PulseAudio::PulseAudioMainloop)
        ...
        set_target_properties(
            PulseAudio::PulseAudioMainloop
            PROPERTIES IMPORTED_LOCATION "${PULSEAUDIO_MAINLOOP_LIBRARY}"
        )
    endif()
