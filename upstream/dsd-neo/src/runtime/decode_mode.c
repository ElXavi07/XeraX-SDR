...
dsd_decode_mode_from_cli_preset(char preset, dsdneoUserDecodeMode* out_mode) {
    ...
    switch (preset) {
        ...
        case 'U': *out_mode = DSDCFG_MODE_AM; return 0;
        case 'W': *out_mode = DSDCFG_MODE_WFM; return 0;
...
decode_mode_dispatch(dsdneoUserDecodeMode mode, dsdDecodePresetProfile profile, dsd_opts* opts, dsd_state* state) {
    ...
    switch (mode) {
        ...
        case DSDCFG_MODE_AM:
            decode_mode_apply_analog(opts, state); opts->analog_demod_mode = 1;
            DSD_SNPRINTF(opts->output_name, sizeof(opts->output_name), "%s", "AM"); return 0;
        case DSDCFG_MODE_WFM:
            decode_mode_apply_analog(opts, state); opts->analog_demod_mode = 2;
            DSD_SNPRINTF(opts->output_name, sizeof(opts->output_name), "%s", "WFM mono"); return 0;
        default: return -1;
    }
}
...
dsd_apply_decode_mode_preset(dsdneoUserDecodeMode mode, dsdDecodePresetProfile profile, dsd_opts* opts,
    ...
    if (mode != DSDCFG_MODE_ANALOG && mode != DSDCFG_MODE_AM && mode != DSDCFG_MODE_WFM) {
        if (opts->analog_only) {
            opts->monitor_input_audio = 0;
        }
        opts->analog_only = 0;
        opts->analog_demod_mode = 0;
    }
    return 0;
}
...
dsd_infer_decode_mode_preset_exact(const dsd_opts* opts) {
    ...
    if (opts->analog_only && opts->monitor_input_audio) {
        return opts->analog_demod_mode == 1 ? DSDCFG_MODE_AM : opts->analog_demod_mode == 2 ? DSDCFG_MODE_WFM : DSDCFG_MODE_ANALOG;
    }
