...
static const rr_field_def k_conv_catalog_fields[] = {
    RR_INT(dsd_rr_conventional_entry, id, "scid"), RR_INT(dsd_rr_conventional_entry, id, "aid"),
    RR_INT(dsd_rr_conventional_entry, id, "mode"),
    RR_STR(dsd_rr_conventional_entry, name, "scName"), RR_STR(dsd_rr_conventional_entry, name, "aName"),
    RR_STR(dsd_rr_conventional_entry, name, "modeName"),
    RR_DEC(dsd_rr_conventional_entry, lat, "lat"), RR_DEC(dsd_rr_conventional_entry, lon, "lon")
};
static const rr_field_def k_conv_frequency_fields[] = {
    RR_INT(dsd_rr_conventional_entry, id, "fid"), RR_INT(dsd_rr_conventional_entry, encrypted, "enc"),
    RR_MHZ(dsd_rr_conventional_entry, frequency_hz, "out"), RR_STR(dsd_rr_conventional_entry, name, "alpha"),
    RR_STR(dsd_rr_conventional_entry, description, "descr"), RR_STR(dsd_rr_conventional_entry, mode, "mode"),
    RR_STR(dsd_rr_conventional_entry, tone, "tone"), RR_STR(dsd_rr_conventional_entry, color_code, "colorCode"),
    RR_STR(dsd_rr_conventional_entry, talkgroup, "tg"), RR_STR(dsd_rr_conventional_entry, slot, "slot")
};
...
typedef struct {
    ...
    dsd_rr_conventional_entry conventional;
...
static const rr_shape_desc k_shapes[] = {
    [RR_SHAPE_CONV_CATEGORIES] = RR_SHAPE_LIST("subcats", conventional, k_conv_catalog_fields, dsd_rr_conventional_list, dsd_rr_conventional_entry),
    [RR_SHAPE_CONV_AGENCIES] = RR_SHAPE_LIST("agencyList", conventional, k_conv_catalog_fields, dsd_rr_conventional_list, dsd_rr_conventional_entry),
    [RR_SHAPE_CONV_FREQUENCIES] = RR_SHAPE_LIST("return", conventional, k_conv_frequency_fields, dsd_rr_conventional_list, dsd_rr_conventional_entry),
    [RR_SHAPE_CONV_MODES] = RR_SHAPE_LIST("return", conventional, k_conv_catalog_fields, dsd_rr_conventional_list, dsd_rr_conventional_entry),
...
RR_SCRATCH_FITS(conventional, dsd_rr_conventional_entry);
...
RR_LIST_HOLDS(dsd_rr_conventional_list, dsd_rr_conventional_entry);
...
rr_apply_leaf(rr_parse_ctx* ctx, const char* name, int depth) {
    ...
    if (ctx->shape == RR_SHAPE_CONV_FREQUENCIES && ctx->record_depth >= 0 && strcmp(name, "tagDescr") == 0) {
        size_t used = strlen(ctx->conventional.tags);
        if (used < sizeof(ctx->conventional.tags) - 3) {
            DSD_SNPRINTF(ctx->conventional.tags + used, sizeof(ctx->conventional.tags) - used, "%s%s", used ? "; " : "", ctx->text);
        }
    }
