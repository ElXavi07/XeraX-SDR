...
typedef enum {
    ...
    RR_M_SUPPORT_FLAVOR,
    RR_M_SUPPORT_VOICE,
    RR_M_CONV_COUNTY_CATEGORIES, RR_M_CONV_COUNTY_AGENCIES, RR_M_CONV_AGENCY_CATEGORIES,
    RR_M_CONV_FREQUENCIES, RR_M_CONV_MODES
} rr_method;
...
static const rr_method_def k_methods[] = {
    [RR_M_CONV_COUNTY_CATEGORIES] = {"getCountyInfo", "ctid", RR_SHAPE_CONV_CATEGORIES, 1, sizeof(dsd_rr_conventional_list)},
    [RR_M_CONV_COUNTY_AGENCIES] = {"getCountyInfo", "ctid", RR_SHAPE_CONV_AGENCIES, 1, sizeof(dsd_rr_conventional_list)},
    [RR_M_CONV_AGENCY_CATEGORIES] = {"getAgencyInfo", "aid", RR_SHAPE_CONV_CATEGORIES, 1, sizeof(dsd_rr_conventional_list)},
    [RR_M_CONV_FREQUENCIES] = {"getSubcatFreqs", "scid", RR_SHAPE_CONV_FREQUENCIES, 1, sizeof(dsd_rr_conventional_list)},
    [RR_M_CONV_MODES] = {"getMode", "mode", RR_SHAPE_CONV_MODES, 1, sizeof(dsd_rr_conventional_list)},
...
rr_free_sink(rr_method method, void* sink) {
    ...
    switch (k_methods[method].shape) {
        case RR_SHAPE_CONV_CATEGORIES:
        case RR_SHAPE_CONV_AGENCIES:
        case RR_SHAPE_CONV_FREQUENCIES:
        case RR_SHAPE_CONV_MODES: free(((dsd_rr_conventional_list*)sink)->items); break;
...
uint64_t dsd_rr_fetch_conventional(dsd_rr_client* client, const dsd_rr_auth* auth, dsd_rr_conventional_kind kind,
                                    int id, dsd_rr_done_cb cb, void* user) {
    if ((unsigned)kind > DSD_RR_CONV_MODES || id < 0 || (id == 0 && kind != DSD_RR_CONV_MODES)) return 0;
    return rr_submit(client, auth, (rr_method)(RR_M_CONV_COUNTY_CATEGORIES + kind), id, cb, user);
}
