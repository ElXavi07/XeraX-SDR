 ...
 *
 * Android device IDs are supplied by the platform router. Output changes are
 * consumed by each stream's writer; input routing is never changed by that API.
 */
...
static atomic_int s_requested_output_device_id = 0;
static atomic_uint s_output_route_generation = 0;
static atomic_int s_output_test_running = 0;
static atomic_int s_output_test_generation = 0;

int
dsd_audio_set_output_device_id(int device_id) {
    if (device_id < 0) {
        return -1;
    }
    atomic_store(&s_requested_output_device_id, device_id);
    atomic_fetch_add(&s_output_route_generation, 1);
    return 0;
}
...
aaudio_build_and_open(const dsd_audio_stream* stream, int32_t sample_rate, AAudioStream** out) {
    ...
    if (!stream->is_input) {
        ...
        AAudioStreamBuilder_setDeviceId(builder, stream->requested_device_id);
...
aaudio_stream_open(dsd_audio_stream* stream) {
    if (!stream->is_input) {
        /* Snapshot once so rate fallback and deeper-buffer retries use the same route. */
        stream->output_route_generation = atomic_load(&s_output_route_generation);
        stream->requested_device_id = atomic_load(&s_requested_output_device_id);
    }
    ...
    if (!stream->is_input) {
        ...
        DSD_FPRINTF(stderr, "AAudio output opened: requested=%d actual=%d input=%dHz/%dch device=%dHz/%dch\n",
                    stream->requested_device_id, AAudioStream_getDeviceId(handle), stream->sample_rate,
                    stream->channels, device_sample_rate, device_channels);
    }
    return 0;
}
...
aaudio_recover_handle(dsd_audio_stream* stream) {
    if (!stream->is_input && stream->output_route_generation != atomic_load(&s_output_route_generation)) {
        /* This runs only on the stream's existing writer, never concurrently
         * with a device write. A new user choice gets one immediate attempt;
         * failures after that retain the ordinary bounded reopen backoff. */
        aaudio_discard_handle(stream->handle);
        stream->handle = NULL;
...
aaudio_write_device_frames(dsd_audio_stream* stream, const int16_t* buf, size_t frames) {
    ...
    while (remaining > 0) {
        ...
        if (res > 0) {
            if (!stream->diagnostic_output) dsd_audio_note_output((size_t)res, stream->device_sample_rate);
...
aaudio_write_frames(dsd_audio_stream* stream, const int16_t* buf, size_t frames) {
    if (!stream->diagnostic_output && dsd_audio_live_suppressed()) return 0;
    if (frames == 0) {
        return 0;
    }
    ...
    atomic_store(&s_output_device_id, AAudioStream_getDeviceId(stream->handle));
    if (!stream->needs_convert) {
...
int dsd_audio_output_test_token(void) { return atomic_load(&s_output_test_generation); }
void dsd_audio_cancel_output_test(void) { atomic_fetch_add(&s_output_test_generation, 1); }

int dsd_audio_test_output(int token) {
    int expected = 0;
    if (!atomic_compare_exchange_strong(&s_output_test_running, &expected, 1)) return -1;
    int result = token == atomic_load(&s_output_test_generation) ? 0 : -2;
    ...
    for (int part = 0; part < 2 && result == 0; ++part) {
        ...
        for (int chunk = 0; chunk < 30; ++chunk) {
            if (token != atomic_load(&s_output_test_generation)) { result = -2; break; }
            ...
            const uint64_t before = stream->device_frames;
            if (aaudio_write_frames(stream, pcm, (size_t)frames) < 0 || stream->device_frames == before) {
                result = -1; break;
            }
        }
        if (result == 0 && dsd_audio_drain(stream) != 0) result = -1;
        dsd_audio_close(stream);
    }
    atomic_store(&s_output_test_running, 0);
    return result;
}
