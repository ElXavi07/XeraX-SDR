...
dsd_am_demod(struct demod_state* fm) {
    const int n = fm->lp_len / 2;
    const float alpha = 1.0f - expf(-6.28318530718f * 30.0f / (float)(fm->rate_out > 0 ? fm->rate_out : 48000));
    float dc = fm->am_dc;
    for (int i = 0; i < n; ++i) {
        const float envelope = hypotf(fm->lowpassed[i * 2], fm->lowpassed[i * 2 + 1]);
        dc += alpha * (envelope - dc);
        fm->result[i] = envelope - dc;
    }
    fm->am_dc = dc;
    fm->result_len = n;
}
...
full_demod_run_cqpsk_chain(struct demod_state* d) {
    ...
    cqpsk_rms_agc(d);
    dsd_equalizer_process(d->lowpassed, static_cast<size_t>(d->lp_len), d->ted_sps);
...
full_demod_run_non_cqpsk_chain(struct demod_state* d) {
    ...
    if (d->mode_demod != &dsd_am_demod) iq_dc_block(d);
...
full_demod_apply_iq_balance(struct demod_state* d) {
    if (d->mode_demod == &dsd_am_demod || !d->iqbal_enable || d->cqpsk_enable || d->channel_squelched || !d->lowpassed || d->lp_len < 2) {
        return;
    }
...
full_demod(struct demod_state* d) {
    ...
    dsd_trial_process(d->lowpassed, static_cast<size_t>(d->lp_len), d->rate_out);
