...
rtl_demod_config_from_env_and_opts(struct demod_state* demod, const dsd_opts* opts) {
    ...
    if (opts->analog_only) {
        ...
        demod->mode_demod = opts->analog_demod_mode == 1 ? &dsd_am_demod : &dsd_fm_demod;
        demod->deemph = opts->analog_demod_mode != 1;
        if (opts->analog_demod_mode == 2) {
            demod->channel_lpf_enable = 0;
