...
struct rtl_device {
    ...
    uint32_t freq = 0U;
    uint32_t rate = 0U;
    std::atomic<uint32_t> bank_frequency{0}, bank_rate{0};
    int gain = 0;
    uint32_t buf_len = 0U;
    int ppm_error = 0;
    int offset_tuning = 0;
...
rtl_submit_capture_bytes(struct rtl_device* s, const void* data, size_t bytes) {
    if (s && (s->backend == RTL_BACKEND_USB || s->backend == RTL_BACKEND_TCP))
        dsd_channel_feed_cu8(static_cast<const unsigned char*>(data), bytes, s->bank_frequency, s->bank_rate);
...
rtlsdr_callback(unsigned char* buf, uint32_t len, void* ctx) {
    ...
    /* Handle muting by discarding muted samples entirely. Feeding midpoint
     * bytes through bias subtraction would create zero-magnitude samples that
     * corrupt the Costas loop and TED after the retune gate opens.
     *
...
static DSD_THREAD_RETURN_TYPE
    ...
    __stdcall
#endif
    hackrf_monitor(void* context) {
    auto* dev = static_cast<rtl_device*>(context);
    while (dev->run.load(std::memory_order_acquire) && !dsd_exitflag_load()) {
        if (!xerax_hackrf_running(dev->hackrf)) {
            if (!dev->run.load(std::memory_order_acquire) || dsd_exitflag_load()) {
                break;
            }
            dsd_input_failure_report(DSD_INPUT_FAILURE_DEVICE, -1);
            DSD_FPRINTF(stderr, "HackRF stream stopped unexpectedly.\n");
            dsd_exitflag_store(1);
            replay_signal_input_waiters(dev);
            break;
        }
    ...
    (void)xerax_hackrf_stop(dev->hackrf);
    dev->run.store(0, std::memory_order_release);
    DSD_THREAD_RETURN;
}
...
static int rtl_tcp_recv_is_timeout(int r);

/* Allow a server time to finish the previous client, without losing partial
 * headers on receive timeouts or blocking Stop. This applies to reconnects too. */
static int
rtl_tcp_skip_header(struct rtl_device* s) {
    (void)dsd_socket_set_recv_timeout(s->sockfd, 200U);
    const uint64_t deadline = dsd_time_monotonic_ns() + 6000000000ULL;
    const bool valid = rtl_tcp_read_header([s, deadline](uint8_t* dst, size_t count) {
        while (s->run.load() && !dsd_exitflag_load() && dsd_time_monotonic_ns() < deadline) {
            const int n = dsd_socket_recv(s->sockfd, dst, count, 0);
            if (n >= 0 || !rtl_tcp_recv_is_timeout(n)) return n;
        }
        return -1;
    });
    const dsdneoRuntimeConfig* cfg = dsd_neo_get_config();
    const int timeout = cfg && cfg->tcp_rcvtimeo_ms > 0 ? cfg->tcp_rcvtimeo_ms : 2000;
    (void)dsd_socket_set_recv_timeout(s->sockfd, static_cast<unsigned>(timeout));
    if (!valid) DSD_FPRINTF(stderr, "rtl_tcp: invalid or incomplete 12-byte header\n");
    return valid ? 1 : 0;
}

static void
rtl_tcp_report_stream_failure(struct rtl_device* s) {
    if (!s || !s->run.load() || dsd_exitflag_load()) return;
    dsd_input_failure_report(DSD_INPUT_FAILURE_NETWORK, 0);
    DSD_FPRINTF(stderr, "rtl_tcp: no usable radio stream; check the server and restart its USB receiver.\n");
    dsd_exitflag_store(1);
    replay_signal_input_waiters(s);
}
...
rtl_tcp_init_loop_state(struct rtl_device* s, const dsdneoRuntimeConfig* cfg, struct rtl_tcp_loop_state* st) {
    ...
    if (!rtl_tcp_skip_header(s)) { free(st->u8); st->u8 = nullptr; return 0; }
...
rtl_tcp_try_reconnect_once(struct rtl_device* s, const struct rtl_tcp_reconnect_snapshot* snap,
    ...
    if (!rtl_tcp_skip_header(s)) { rtl_tcp_close_socket_if_open(s); return 0; }
...
rtl_tcp_reconnect_after_stall(struct rtl_device* s, struct rtl_tcp_loop_state* st, int* out_r) {
    ...
    int attempt = 0;
    const uint64_t deadline = dsd_time_monotonic_ns() + 20000000000ULL;
    while (s->run.load() && dsd_exitflag_load() == 0 && dsd_time_monotonic_ns() < deadline) {
        attempt++;
    ...
    rtl_tcp_report_stream_failure(s);
    return 0;
}
...
rtl_usb_verify_sample_rate(void* opaque) {
    ...
    // cppcheck-suppress knownConditionTrueFalse -- The no-RTL optional build always reports no sample rate.
...
rtl_device_preopened_fd_is_set(void) {
    return (g_preopened_usb_fd.load(std::memory_order_acquire) >= 0 || xerax_hackrf_fd_set()) ? 1 : 0;
}

int
rtl_device_preopened_fd_in_use(void) {
    return g_preopened_usb_fd_in_use.load(std::memory_order_acquire) || xerax_hackrf_fd_in_use();
}
...
rtl_device_create(int dev_index, struct input_ring_state* input_ring) {
    ...
    if (xerax_hackrf_fd_set()) {
        dev->backend = RTL_BACKEND_HACKRF;
        dev->hackrf = xerax_hackrf_open(rtlsdr_callback, dev);
        if (!dev->hackrf) {
            rtl_device_cleanup_common_state(dev); free(dev); return nullptr;
        }
        return dev;
    }
...
rtl_device_request_thread_stop(struct rtl_device* dev) {
    ...
    if (dev->backend == RTL_BACKEND_HACKRF || dev->backend == RTL_BACKEND_AIRSPY || dev->backend == RTL_BACKEND_TCP || dev->backend == RTL_BACKEND_SOAPY
        || dev->backend == RTL_BACKEND_IQ_REPLAY) {
...
rtl_device_close_backend_resources(struct rtl_device* dev) {
    if (dev && dev->hackrf) { xerax_hackrf_close(dev->hackrf); dev->hackrf = nullptr; }
...
rtl_device_set_frequency(struct rtl_device* dev, uint32_t frequency) {
    ...
    } else if (dev->backend == RTL_BACKEND_HACKRF) {
        rc = xerax_hackrf_frequency(dev->hackrf, frequency);
    ...
    if (rc == 0) {
        dev->freq = frequency;
        dev->bank_frequency = frequency;
        if (dev->backend == RTL_BACKEND_HACKRF || dev->backend == RTL_BACKEND_USB || dev->backend == RTL_BACKEND_TCP) {
...
rtl_device_set_sample_rate(struct rtl_device* dev, uint32_t samp_rate) {
    if (dev && dev->hackrf) {
        int rc = xerax_hackrf_rate(dev->hackrf, samp_rate);
        if (!rc) { dev->rate = samp_rate; dev->bank_rate = samp_rate; }
        return rc;
    }
    if (dev && dev->backend == RTL_BACKEND_AIRSPY) {
...
rtl_device_get_sample_rate(struct rtl_device* dev) {
    ...
    if (dev->backend == RTL_BACKEND_USB) {
        ...
        // cppcheck-suppress unsignedLessThanZero -- The no-RTL optional build always reports a zero sample rate.
        if (actual > 0U) {
            dev->rate = actual;
            dev->bank_rate = actual;
        }
        return (int)actual;
    }
    if (dev->backend == RTL_BACKEND_IQ_REPLAY) {
...
rtl_device_set_gain(struct rtl_device* dev, int gain) {
    if (dev && dev->hackrf) {
        const int applied = xerax_hackrf_gain(dev->hackrf, gain);
        if (applied < 0) return applied;
        dev->gain = applied; dev->agc_mode = 0; return 0;
    }
    if (dev && dev->backend == RTL_BACKEND_AIRSPY) {
        (void)gain;
...
rtl_device_set_gain_nearest(struct rtl_device* dev, int target_tenth_db) {
    if (dev && dev->hackrf) return rtl_device_set_gain(dev, target_tenth_db);
    if (dev && dev->backend == RTL_BACKEND_AIRSPY) {
...
rtl_device_get_tuner_gain(struct rtl_device* dev) {
    if (dev && dev->hackrf) return dev->gain;
    if (dev && dev->backend == RTL_BACKEND_AIRSPY) {
        return DSD_ERR_NOT_SUPPORTED;
    }
    if (!dev) {
...
rtl_device_set_ppm(struct rtl_device* dev, int ppm_error) {
    if (dev && dev->hackrf) return ppm_error == 0 ? 0 : DSD_ERR_NOT_SUPPORTED;
    if (!dev) {
        return -1;
    }
...
rtl_device_set_direct_sampling(struct rtl_device* dev, int on) {
    if (dev && dev->hackrf) return on == 0 ? 0 : DSD_ERR_NOT_SUPPORTED;
    if (!dev) {
        return -1;
    }
    int rc = -1;
...
rtl_device_set_tuner_bandwidth(struct rtl_device* dev, uint32_t bw_hz) {
    if (dev && dev->hackrf) return DSD_ERR_NOT_SUPPORTED;
    if (!dev) {
        return -1;
    }
    if (dev->backend == RTL_BACKEND_USB) {
...
rtl_device_start_async(struct rtl_device* dev, uint32_t buf_len) {
    ...
    int r = 0;
    if (dev->backend == RTL_BACKEND_HACKRF) {
        dev->run.store(1, std::memory_order_release);
        r = xerax_hackrf_start(dev->hackrf);
        if (!r) r = dsd_thread_create(&dev->thread, hackrf_monitor, dev);
        if (r) (void)xerax_hackrf_stop(dev->hackrf);
    } else if (dev->backend == RTL_BACKEND_AIRSPY) {
        dev->run.store(1, std::memory_order_release);
...
rtl_device_stop_async(struct rtl_device* dev) {
    ...
    } else if (dev->backend == RTL_BACKEND_HACKRF || dev->backend == RTL_BACKEND_SOAPY || dev->backend == RTL_BACKEND_AIRSPY) {
        dev->run.store(0);
    } else {
        return -1;
    }
...
rtl_device_set_bias_tee(struct rtl_device* dev, int on) {
    if (dev && dev->hackrf) return xerax_hackrf_bias(dev->hackrf, on);
    if (!dev) {
        return -1;
    }
