...
int
keyring_scalar_present(const dsd_state* state, int key_id) {
    return keyring_rkey_index_valid(state, key_id)
           && (state->rkey_array_loaded[key_id] != 0U || state->rkey_array[key_id] != 0ULL);
}
...
keyring_activate_slot_with_kid(dsd_state* state, int slot, int key_id) {
    ...
    state->scalar_key_present[slot] = keyring_scalar_present(state, key_id);
...
keyring_kid_satisfies_need(const dsd_state* state, int key_id, dsd_key_material_need need) {
    ...
    switch (need) {
        case DSD_KEY_NEED_SCALAR: return keyring_scalar_present(state, key_id);
