...
dsd_dmr_slot_key_material(const dsd_state* state, int slot, int key_id, int mapped) {
    ...
    if (mapped) {
        material.scalar_present = keyring_scalar_present(state, key_id);
...
dsd_audio_group_gate_slot(const dsd_opts* opts, const dsd_state* state, int slot, unsigned long tg, int enc_in,
    ...
    const int cc = state->dmr_confidence_locked ? state->dmr_confidence_color_code
        : state->receiver_ms_color_known ? (int)state->dmr_color_code : -1;
    if (!dsd_receiver_digital_allowed(opts->rtlsdr_center_freq, DSD_SYNC_IS_DMR(state->synctype), cc,
        (slot < 0 ? state->currentslot : slot) + 1, ota_tg)) enc = 1;
    *enc_out = enc;
    return 0;
}
