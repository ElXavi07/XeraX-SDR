...
DecryptionProfilesModel::directTypes(const QString& protocol) const {
    const auto type = [](const char* id, const char* label) { return QVariantMap{{"id", id}, {"label", QObject::tr(label)}}; };
    if (protocol == "m17") {
    ...
    if (protocol == "p25" || protocol == "dmr" || protocol == "mixed")
        result << type("adp", "ADP / ARC4 (40-bit)");
...
managedShape(const QString& kind, const QString& material) {
    ...
    const QMap<QString, int> widths{{"adp", 10}, {"aes128", 32}, {"tdea", 48}, {"aes256", 64}};
...
managedKindAllowed(const QString& kind, const QString& protocol, bool destination) {
    ...
    if (kind == "adp") return protocol == "p25" || protocol == "dmr";
    if (kind == "basic") {
        return protocol == "dmr";
    }
    if (kind == "scrambler") {
...
appendManagedKey(ManagedCollection& collection, const QVariantMap& key, const QString& protocol, QString* error) {
    ...
    const quint32 maximum = destination ? (protocol == "nxdn" ? 65535U : 0xFFFFFFU)
                                       : protocol == "nxdn" ? 63U : protocol == "dmr" ? 255U : 65535U;
...
struct ProfileCompiler {
    ...
    direct(const QVariantList& types) {
        ...
        const bool valid = type == "adp" ? QRegularExpression("^[0-9A-F]{10}$").match(session_args_key_hex_normalize(value)).hasMatch()
                                         : session_args_key_valid(type, value);
        if ((!allowed && !legacy) || !valid) {
            if (type == "adp") return fail(QObject::tr("ADP / ARC4 needs exactly 10 hexadecimal digits (40 bits), including leading zeros."));
        ...
        compiled.insert("encKeyType", type == "adp" ? QString("rc4") : type);
        return true;
    }
