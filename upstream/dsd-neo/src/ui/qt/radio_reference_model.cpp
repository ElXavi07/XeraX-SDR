...
RadioReferenceModel::convertResult(const Request& request, void* result, Reply* reply) {
    ...
    switch (request.kind) {
        case FetchConvCategories:
        case FetchConvAgencies:
        case FetchConvFrequencies:
        case FetchConvModes: {
            auto* rows = static_cast<dsd_rr_conventional_list*>(result);
            for (size_t i = 0; i < rows->count; ++i) {
                const auto& row = rows->items[i];
                const bool position = site_position_valid(row.lat, row.lon);
                reply->list.append(QVariantMap{{"id", row.id}, {"name", field(row.name)},
                    {"description", field(row.description)}, {"mode", field(row.mode)},
                    {"freqMhz", row.frequency_hz ? hz_to_mhz_text(row.frequency_hz) : QString()},
                    {"tone", field(row.tone)}, {"colorCode", field(row.color_code)},
                    {"talkgroup", field(row.talkgroup)}, {"slot", field(row.slot)}, {"tags", field(row.tags)},
                    {"encrypted", row.encrypted}, {"lat", row.lat}, {"lon", row.lon},
                    {"distanceMi", position ? site_distance_km(33.7206, -116.2156, row.lat, row.lon) / 1.609344 : -1}});
            }
            free(rows->items); free(rows); break;
        }
...
void RadioReferenceModel::loadConventional(int kind, int id) {
    if (kind < 0 || kind > 2 || id <= 0) return;
    startBatch(tr("Loading conventional radio channels…"));
    m_conventionalFrequencies.clear();
    if (kind != 2) m_conventionalCategories.clear();
    auto queue = [&](Fetch fetch, dsd_rr_conventional_kind method, int argument) {
        Request* request = beginFetch(fetch);
        if (request) endFetch(dsd_rr_fetch_conventional(m_client, &request->auth, method, argument, &onFetchDone, request), request);
    };
    if (m_conventionalModes.isEmpty()) queue(FetchConvModes, DSD_RR_CONV_MODES, 0);
    if (kind == 0) {
        m_conventionalAgencies.clear();
        queue(FetchConvCategories, DSD_RR_CONV_COUNTY_CATEGORIES, id);
        queue(FetchConvAgencies, DSD_RR_CONV_COUNTY_AGENCIES, id);
    } else if (kind == 1) queue(FetchConvCategories, DSD_RR_CONV_AGENCY_CATEGORIES, id);
    else queue(FetchConvFrequencies, DSD_RR_CONV_FREQUENCIES, id);
    Q_EMIT listsChanged();
}
...
RadioReferenceModel::applyListReply(const Reply& reply) {
    switch (reply.kind) {
        case FetchConvCategories: m_conventionalCategories = reply.list; break;
        case FetchConvAgencies: m_conventionalAgencies = reply.list; break;
        case FetchConvModes:
            for (const auto& item : reply.list) {
                auto row = item.toMap(); m_conventionalModes[QString::number(row.value("id").toInt())] = row.value("name").toString();
            }
            break;
        case FetchConvFrequencies:
            m_conventionalFrequencies.clear();
            for (const auto& item : reply.list) {
                auto row = item.toMap();
                const QString raw = row.value("mode").toString();
                const QString mode = m_conventionalModes.value(raw, raw).toUpper();
                QString protocol;
                if (mode == "FM" || mode == "FMN" || mode == "NFM") protocol = "nfm";
                else if (mode == "DMR") protocol = "dmr";
                else if (mode == "P25") protocol = "p25";
                else if (mode == "AM") protocol = "am";
                else if (mode == "WFM" || mode == "FMB") protocol = "wfm";
                // NXDN bandwidth is not implied by a generic NXDN database label.
                row["modeName"] = mode; row["protocol"] = protocol;
                m_conventionalFrequencies.append(row);
            }
            break;
...
RadioReferenceModel::finishSystemLoad() {
    ...
    for (size_t i = 0; i < m_siteData.count; i++) {
        ...
        // XeraX's local preset uses approximate central Indio, never the user's
        // address or a device fix. RR coordinates may be coverage centroids.
        const double indioKm = site.has_position ? site_distance_km(33.7206, -116.2156, site.lat, site.lon) : -1;
        row.insert(QStringLiteral("indioDistanceMi"), indioKm >= 0 ? indioKm / 1.609344 : -1);
