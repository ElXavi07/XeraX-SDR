...
normalize(const QVariantMap& input, const QVariantMap& base = {}) {
    QVariantMap result =
        base.isEmpty()
            ? QVariantMap{{"name", ""},         {"sourceType", "usb"}, {"host", ""},         {"port", 1234},
                          ...
                          {"srcCsvPath", ""},   {"lastHeard", 0}, {"maxVisitMs", -1}}
            : base;
    const QStringList fields{"airspy",         "name",          "sourceType",   "host",          "port",
                             ...
                             "entries",        "isDraft",       "targetSource", "targetsCsvPath", "maxVisitMs"};
    ...
    for (const auto& value : result.value("entries").toList()) {
        ...
        for (const auto& field : {"uid", "kind", "systemUid", "name", "protocol", "freqMhz", "enabled", "priority", "dwellMs",
