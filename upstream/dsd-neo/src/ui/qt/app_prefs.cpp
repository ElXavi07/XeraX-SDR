...
QString
AppPrefs::exploreDecodeFlag() const {
    return m_settings.value(QStringLiteral("explore/decodeFlag"), QStringLiteral("-fa")).toString();
}

void
AppPrefs::setExploreDecodeFlag(const QString& flag) {
    if (flag == exploreDecodeFlag()) return;
    m_settings.setValue(QStringLiteral("explore/decodeFlag"), flag);
    Q_EMIT exploreChanged();
}
