...
append_flag_args(QStringList& args, const QVariantMap& system, const SessionArgPrefs& prefs, double hangtime) {
    ...
    // The guided capture's size bound wins over any app-wide advanced option.
    const QString iqPath = system.value("iqCapturePath").toString();
    if (!iqPath.isEmpty()) args << "--iq-capture" << iqPath << "--iq-capture-max-mb" << "64" << "--iq-capture-format"
        << (system.value("sourceType").toString() == "airspy" ? "cf32" : "cu8");
}
...
QString
session_args_scan_timing_error(const QVariantMap& list) {
    for (const auto& field : {"defaultDwellMs", "defaultHoldMs", "maxVisitMs"}) {
        const bool visit = QString::fromLatin1(field) == QLatin1String("maxVisitMs");
        bool ok = false;
        const int value = list.value(field, visit ? -1 : 0).toString().toInt(&ok);
        const int minimum = visit ? 1000 : 250;
        const int maximum = visit ? 3600000 : 600000;
        if (!ok || !(value == 0 || (visit && value == -1) || (value >= minimum && value <= maximum))) {
            return visit ? QStringLiteral("Maximum visit must be 1000..3600000 ms, 0 for no limit, or empty to inherit.")
                         : QStringLiteral("Dwell and hold must be 250..600000 ms, or 0 to inherit.");
        }
    }
    return {};
}
...
appendScanOptions(QStringList& args, const QVariantMap& list, const QString& csvPath, const SessionArgPrefs& prefs) {
    ...
    // Explicit list settings win over app-wide extra arguments. A missing/-1
    // value preserves the previous release's global-option behavior; 0 disables.
    if (list.value("maxVisitMs", -1).toInt() >= 0) {
        args << QStringLiteral("--scan-max-visit-ms") << list.value("maxVisitMs").toString();
    }
}
...
session_args_scan_build(const QVariantMap& list, const QString& firstFreqMhz, const QString& csvPath,
    ...
    if (validation.isEmpty()) {
        validation = session_args_scan_timing_error(list);
    }
    if (!validation.isEmpty()) {
        if (error) {
            *error = validation;
...
session_args_profile_compatible(const QVariantMap& system) {
    ...
    if (flags.contains("-ft") || flags.contains("-f1") || flags.contains("-f2")) {
        expected = "p25";
    } else if (flags.contains("-fs") || flags.contains("-fr")) {
        expected = "dmr";
    } else if (flags.contains("-fi") || flags.contains("-fn")) {
        expected = "nxdn";
    } else if (flags.contains("-fm")) {
        expected = "dpmr";
...
session_args_build(const QVariantMap& system, const SessionArgPrefs& prefs, SessionArgsError* error) {
    ...
    if(system.value("siteCapture").toBool()) args << "--xerax-site-capture";
    return args;
}
...
SessionArgsBuilder::buildArgs(const QVariantMap& system, SessionArgsError* error) const {
    ...
    if(system.value("workerSession").toBool() || system.value("siteCapture").toBool()) { prefs.extraArgs.clear(); prefs.autoPpm=false; }
...
SessionArgsBuilder::start(const QVariantMap& system, DecoderHost* host) const {
    SessionArgsError error = SessionArgsError::None;
    const QStringList args = buildArgs(system, &error) + CallLibrary::recordingArgs();
