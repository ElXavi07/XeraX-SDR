...
struct DecryptionView {
    ...
    QString
    availability() const {
        if (DSD_SYNC_IS_NXDN(call.protocol) && call.algid == 1 && call.epoch == state->nxdn_search.epoch
            && dsd_nxdn_search_audio_allowed(&state->nxdn_search)) {
            return QStringLiteral("Experimental NXDN candidate in use");
        }
