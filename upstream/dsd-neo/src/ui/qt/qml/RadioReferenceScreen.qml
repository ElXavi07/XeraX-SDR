...
Item {
    ...
    property bool showIndioDistances: false
    property bool conventionalOpen: false
    NavigationLayer {
        surface: conventionalPage
        active: screen.conventionalOpen && screen.visible
        onLeave: screen.conventionalOpen = false
    }
    Loader {
        id: conventionalPage
        anchors.fill: parent
        z: 50
        active: screen.conventionalOpen
        sourceComponent: ConventionalBrowser {
            countyId: screen.browseCtid > 0 ? screen.browseCtid : 215
            onClosed: screen.conventionalOpen = false
        }
    }
    onEachSiteChanged: {
        selectedSites = [];
    ...
    function reset() {
        ...
        screen.siteSearch = "";
        screen.showIndioDistances = false;
    ...
    function findRiverside() {
        if (radioReference.busy || !radioReference.credentialsReady)
            return;
        screen.clearNotice();
        Qt.inputMethod.hide();
        screen.sourceMode = 1;
        ...
        screen.browseCountryName = qsTr("United States");
        screen.browseStid = 6;
        screen.browseStateName = qsTr("California");
        screen.browseCtid = 215;
        screen.browseCountyName = qsTr("Riverside County");
        screen.showIndioDistances = true;
        if (screen.rrLive())
            radioReference.loadCountySystems(215);
    }

    function selectIndioSites() {
        if (radioReference.busy || (!radioReference.trunked && !radioReference.conventional))
            return;
        var selected = [];
        var unknown = 0;
        var sites = radioReference.sites;
        for (var i = 0; i < sites.length; i++) {
            var miles = sites[i].indioDistanceMi;
            if (typeof miles !== "number" || !isFinite(miles) || miles < 0)
                unknown++;
            else if (miles <= 50)
                selected.push(i);
        }
        screen.showIndioDistances = true;
        screen.siteSearch = "";
        // Change selection mode first: its handler clears the old selection.
        if (radioReference.trunked)
            screen.eachSite = true;
        screen.selectedSites = selected;
        screen.notice = qsTr("%1 site(s) selected within 50 mi of Indio. %2 without usable coordinates left unselected. Review before importing.").arg(selected.length).arg(unknown);
        screen.noticeIsProblem = false;
    }
        ...
        Column {
            ...
            UiPanel {
                ...
                Column {
                    ...
                    OutlineButton {
                        objectName: "radioReferenceRiverside"
                        width: parent.width
                        text: qsTr("Indio / Coachella Valley · Riverside County")
                        enabled: !radioReference.busy
                        onClicked: screen.findRiverside()
                    }
                    OutlineButton {
                        width: parent.width
                        text: qsTr("Conventional frequencies · analog and digital")
                        enabled: !radioReference.busy
                        onClicked: screen.conventionalOpen = true
                    }

                    Text {
                        width: parent.width
                        text: qsTr("Open a county system, then select its sites within 50 mi of Indio. County results can include sites farther away.")
                        font.family: Theme.sans
            ...
            UiPanel {
                ...
                Column {
                    ...
                    OutlineButton {
                        objectName: "radioReferenceIndioSites"
                        width: parent.width - 2 * Theme.cardPadding
                        x: Theme.cardPadding
                        text: qsTr("Select sites within 50 mi of Indio")
                        enabled: !radioReference.busy && (radioReference.trunked || radioReference.conventional)
                        onClicked: screen.selectIndioSites()
                    }
                    ...
                    Text {
                        ...
                        topPadding: 8
                        bottomPadding: 8
                        text: qsTr("Uses listed coordinates, which may represent coverage areas. Distance does not guarantee reception. You can adjust the selection below.")
                        font.family: Theme.sans
                        font.pixelSize: Theme.fontSize(12)
                        color: Theme.textSubdued
                        wrapMode: Text.Wrap
                    ...
                    Repeater {
                        ...
                        Item {
                            ...
                            Column {
                                ...
                                Text {
                                    width: parent.width
                                    visible: screen.showIndioDistances
                                    text: {
                                        var miles = siteRow.modelData.site.indioDistanceMi;
                                        return typeof miles === "number" && isFinite(miles) && miles >= 0
                                            ? qsTr("%1 mi from Indio · listed position").arg(miles.toFixed(1))
                                            : qsTr("Distance from Indio unavailable");
                                    }
                                    font.family: Theme.sans
                                    font.pixelSize: Theme.fontSize(12)
                                    color: Theme.textSubdued
