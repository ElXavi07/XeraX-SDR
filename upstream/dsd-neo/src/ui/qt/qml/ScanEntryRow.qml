...
UiPanel {
    ...
    Column {
        ...
        PlexCheckBox {
            objectName: "scanEntryPriority"
            width: parent.width
            text: qsTr("Priority: revisit between ordinary channels")
            checked: row.entry.priority === true
            onToggled: row.changed("priority", checked)
        }
        Row {
            width: parent.width
            spacing: 6
        ...
        PlexComboBox {
            ...
            model: [qsTr("P25"), qsTr("DMR"), qsTr("NXDN48"), qsTr("NXDN96"), qsTr("Analog NFM")]
