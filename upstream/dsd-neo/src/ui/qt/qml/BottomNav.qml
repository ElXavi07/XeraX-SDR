    ...
    Item {
        ...
        Repeater {
            ...
            Item {
                ...
                x: nav.vertical ? 0 : index * nav.width / 4
                y: nav.vertical ? index * Math.max(84, Theme.fontSize(11) + 56) + 12 : 0
                width: nav.vertical ? nav.width : nav.width / 4
                ...
                function choose() {
                    if (enabled && Navigation.allows(nav))
                        nav.selected([0,3,1,2][index]);
                }
                Keys.onSpacePressed: choose()
                Keys.onReturnPressed: choose()
                FocusFrame {}
                readonly property bool active: nav.currentIndex === [0,3,1,2][index]
