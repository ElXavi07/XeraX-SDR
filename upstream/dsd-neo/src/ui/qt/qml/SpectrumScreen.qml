...
Item {
    ...
    readonly property bool tunerHeld: (metrics ? metrics.tunerControlled : false) || (typeof rangeScanner !== "undefined" && rangeScanner.active)
    ...
    readonly property real readoutHz: swipeActive || swipePending ? swipeHz : bandRail.displayHz
    readonly property bool analogMode: ["-fA", "-fU", "-fW"].some(function(f) { return metrics.decodeMode === commands.decodeModeForFlag(f); })
    ...
    property bool fineSwipe: true
    property int tuningStepHz: 12500
    property bool swipeActive: false
    property bool swipePending: false
    property real swipeHz: 0
    property real swipeStartHz: 0
    property real swipeSpanHz: 0
    property bool dragCancelled: false
    property bool dragTunes: false
    ...
    function beginDrag() {
        ...
        cancelSwipe();
        dragCancelled = false;
        dragTunes = fineSwipe && !viewOnly;
        panStartOffsetHz = spectrum.viewOffsetHz;
        swipeStartHz = tunedHz;
        swipeHz = tunedHz;
        // At most 24 channel steps per screen: precise even at the widest zoom.
        swipeSpanHz = Math.min(spectrum.viewSpanHz, tuningStepHz * 24);
        swipeActive = dragTunes;
    }
    function updateDrag(dx, width) {
        if (width <= 0 || dragCancelled) return;
        if (!dragTunes) { applyPan(dx, width); return; }
        var delta = Math.round((-dx / width * swipeSpanHz) / tuningStepHz) * tuningStepHz;
        swipeHz = swipeStartHz + delta;
    }
    function endDrag(cancelled) {
        var target = swipeHz;
        var shouldTune = dragTunes && swipeActive && !cancelled && !dragCancelled && spectrum.hasData;
        swipeActive = false;
        if (shouldTune && target !== swipeStartHz && tuneTo(target)) {
            swipeHz = target;
            swipePending = true;
            swipeSettle.restart();
        }
    ...
    function fineStep(direction) {
        stopSweep();
        tuneTo(tunedHz + direction * tuningStepHz);
    }
    onTunedHzChanged: {
        if (swipePending && Math.abs(tunedHz - swipeHz) < 1) cancelSwipe();
    }
    Timer {
        id: swipeSettle
        interval: 2000
        onTriggered: {
            screen.swipePending = false;
            screen.hint = qsTr("Tune not confirmed; showing receiver frequency");
        }
    }
    ...
    function tuneTo(hz) {
        ...
        screen.hint = "";
        cancelSwipe();
        var accepted = commands.manualTuneHz(Math.round(hz));
        if (!accepted) screen.hint = qsTr("Receiver did not accept tuning");
        return accepted;
    }
    ...
    function sweepTick() {
        if (!screen.sweeping)
            return;
        var energy = spectrum.channelExcessDb(metrics.decodeMode === commands.decodeModeForFlag("-fW") ? 180000 : 12500);
        if ((screen.analogMode && isFinite(energy) && energy >= 10) || (!screen.analogMode && metrics.syncedHere)) {
            screen.stopSweep();
            screen.hint = screen.analogMode ? qsTr("Energy found at %1 · %2 dB above background. Listen and save if useful.").arg(Util.fmtMhz(screen.tunedHz)).arg(energy.toFixed(1))
                : qsTr("Found something at %1").arg(Util.fmtMhz(screen.tunedHz));
            return;
        }
        screen.stepBy(1);
    }
    ...
    Item {
        ...
        Column {
            ...
            Row {
                ...
                Text {
                    ...
                    font.pixelSize: Theme.fontSize(screen.width < 420 ? 17 : 21)
                    ...
                    color: (screen.swipeActive || screen.swipePending || bandRail.dragging || bandRail.settling) ? Theme.cyan : Theme.textPrimary
                }
        ...
        Rectangle {
            ...
            Text {
                ...
                text: screen.viewOnly ? qsTr("VIEW ONLY") : screen.sweeping ? qsTr("SWEEPING") : (screen.swipeActive ? qsTr("RELEASE") : qsTr("TUNE"))
                font.family: Theme.mono
    ...
    Item {
            ...
            Row {
                ...
                Row {
                    ...
                    Text {
                        ...
                        text: screen.analogMode ? (metrics.decodeMode === commands.decodeModeForFlag("-fU") ? "AM" : metrics.decodeMode === commands.decodeModeForFlag("-fW") ? "WFM MONO" : "NFM") : metrics.syncedHere ? metrics.syncLabel.toUpperCase() : qsTr("NO SYNC")
                        font.family: Theme.mono
                        font.pixelSize: Theme.fontSize(11)
        ...
        Item {
            ...
            TunerMarker {
                ...
                tunedHz: screen.swipeActive || screen.swipePending ? screen.swipeHz : screen.tunedHz
            ...
            DragHandler {
                ...
                onActiveChanged: {
                    ...
                    Qt.callLater(function() { screen.endDrag(screen.dragCancelled || pinch.active); });
                }
                onCanceled: { screen.dragCancelled = true; screen.cancelSwipe(); }
                onActiveTranslationChanged: {
                    if (pan.active)
                        screen.updateDrag(pan.activeTranslation.x, gestureArea.width);
                }
            }
        }
        ...
        Column {
            ...
            Row {
                ...
                spacing: 8
                OutlineButton {
                    objectName: "spectrumFineDown"
                    width: 48
                    text: "−"
                    accessibleName: qsTr("Tune down one channel step")
                    enabled: spectrum.hasData
                    onClicked: screen.fineStep(-1)
                }
                OutlineButton {
                    objectName: "spectrumFineStep"
                    width: parent.width - 2 * 48 - 72 - 3 * 8
                    text: (screen.tuningStepHz / 1000) + " kHz"
                    accessibleName: qsTr("Channel step; tap to change")
                    onClicked: screen.tuningStepHz = screen.tuningStepHz === 6250 ? 12500 : screen.tuningStepHz === 12500 ? 25000 : 6250
                }
                OutlineButton {
                    objectName: "spectrumFineUp"
                    width: 48
                    text: "+"
                    accessibleName: qsTr("Tune up one channel step")
                    enabled: spectrum.hasData
                    onClicked: screen.fineStep(1)
                }
                OutlineButton {
                    objectName: "spectrumSwipeMode"
                    width: 72
                    text: screen.fineSwipe ? qsTr("Fine") : qsTr("Pan")
                    accessibleName: qsTr("Toggle fine swipe tuning or view panning")
                    onClicked: screen.fineSwipe = !screen.fineSwipe
                }
            }
            Text {
                width: parent.width
                text: screen.fineSwipe ? qsTr("Swipe for fine tuning · tap a peak · pinch to zoom") : qsTr("Swipe pans the view · tap a peak to tune")
    ...
    ModalSheet {
        ...
        Text {
            width: parent.width
            text: typeof rangeScanner !== "undefined" && rangeScanner.active ? qsTr("This stops the frequency range scan so you can tune manually.") : metrics.scannerMode ? qsTr("This stops stepping through the channel list for now. The saved system itself is unchanged.") : qsTr("This stops following calls across channels for now. The saved system itself is unchanged.")
            font.family: Theme.sans
