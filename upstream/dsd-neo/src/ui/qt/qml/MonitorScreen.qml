...
Item {
    ...
    signal openAudioCheck
    signal openReceiverTools
    signal openSpectrum
    ...
    Item {
        ...
        Column {
            ...
            Text {
                ...
                text: metrics.scanTargetCount > 0 ? qsTr("SCANNING · %1/%2").arg(metrics.scanTargetOrdinal).arg(metrics.scanTargetCount) + (metrics.scanHold ? qsTr(" · HOLD") : "") + (screen.scanTargetName.length > 0 ? " · " + screen.scanTargetName : "") : screen.system ? Util.monitorMeta(Object.assign({},screen.system,{freqMhz: metrics.centerFreqHz>0 ? (metrics.centerFreqHz/1e6).toFixed(6) : screen.system.freqMhz})) : ""
    ...
    UiPanel {
        ...
        Column {
            ...
            Text {
                ...
                text: screen.system && screen.system.siteCapture ? qsTr("Four-channel reception") : screen.system && ["-fA", "-fU", "-fW"].indexOf(screen.system.decodeFlag) >= 0 ? qsTr("Analog listening") : metrics.syncedHere ? qsTr("waiting for a call…") : qsTr("waiting for signal…")
                font.family: Theme.sans
                font.pixelSize: Theme.fontSize(20)
                color: Theme.textSubdued
            }
    ...
    PlexFlickable {
        ...
        Column {
            ...
            Flow {
                ...
                OutlineButton { width: (parent.width - 8) / 2; text: typeof rangeScanner !== "undefined" && rangeScanner.active ? qsTr("Scan controls") : qsTr("Receiver tools"); onClicked: screen.openReceiverTools() }
                OutlineButton { width: (parent.width - 8) / 2
                    text: typeof receiverAssistant !== "undefined" && receiverAssistant.status.replaying ? qsTr("Return to live") : qsTr("Replay 30 s")
                    enabled: receiverTools.replaySeconds > 0
                    onClicked: { if (receiverAssistant.status.replaying) receiverTools.stopPlayback(); else receiverTools.playReplay(30); }
                }
            }
            OutlineButton {
                objectName: "monitorAudioCheck"
                width: parent.width
                visible: typeof receiverAssistant !== "undefined"
                text: qsTr("Check audio")
                onClicked: screen.openAudioCheck()
            }
            Text {
                width: parent.width
                text: typeof receiverAssistant !== "undefined" ? receiverAssistant.status.audioText || "" : ""
                visible: text.length > 0
                color: Theme.textSecondary
                font.pixelSize: Theme.fontSize(12)
                wrapMode: Text.Wrap
            ...
            Text {
                width: parent.width
                visible: typeof receiverExpansion !== "undefined" && !!receiverExpansion.status.nxdnSearch
                text: typeof receiverExpansion !== "undefined" && receiverExpansion.status.nxdnSearchStatus===3
                    ? qsTr("Experimental NXDN candidate in use. Verify that speech is clear.")
                    : qsTr("NXDN search enabled · see Receiver lab → Reception")
                color: Theme.textSecondary; font.pixelSize: Theme.fontSize(12); wrapMode: Text.Wrap
            }
            UiPanel {
                id: siteRow
                objectName: "siteRow"
