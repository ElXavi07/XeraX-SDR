...
ModalSheet {
    ...
    readonly property var types: { var language = typeof appLanguage !== "undefined" ? appLanguage.language : "en"; return available ? decryptionProfiles.directTypes(draft.protocol || "mixed") : []; }
    readonly property var vendors: available ? decryptionProfiles.vendorTypes() : []
    readonly property var kinds: draft.protocol === "p25" ? ["adp", "scalar", "aes128", "aes256", "tdea"] : draft.protocol === "dmr" ? ["adp", "scalar", "aes128", "aes256", "basic"] : draft.protocol === "nxdn" ? ["scalar", "aes256", "scrambler"] : ["adp", "scalar", "aes128", "aes256", "tdea", "basic", "scrambler"]
    ...
    PlexTextField {
        id: profileName
        objectName: "decryptionProfileName"
        width: parent.width
    ...
    Column {
        ...
        PlexTextField {
            id: directMaterial
            objectName: "profileDirectMaterial"
            width: parent.width
        ...
        Text {
            width: parent.width; visible: sheet.draft.directType === "adp"
            text: qsTr("ADP / ARC4: enter the supplied 40-bit key as 10 hex digits. P25 Phase 1/2 uses ALG AA; DMR Enhanced Privacy uses ALG 21. Key IDs identify keys; they are not the key itself.")
            wrapMode: Text.Wrap; color: Theme.textSecondary; font.pixelSize: Theme.fontSize(13)
        }
    }
    Column {
        width: parent.width
    ...
    ModalSheet {
        ...
        PlexComboBox {
            ...
            model: sheet.kinds.map(function(kind) { return kind === "adp" ? qsTr("ADP / ARC4 (40-bit)") : kind === "scalar" ? qsTr("RC4 / DES (general)") : kind; })
        }
        PlexComboBox {
            id: keyLookup
            width: parent.width
        ...
        Text {
            width: parent.width; visible: sheet.kinds[keyKind.currentIndex] === "adp"
            text: qsTr("Use 10 hexadecimal digits for the ADP / ARC4 key, including leading zeros.")
            wrapMode: Text.Wrap; color: Theme.textSecondary; font.pixelSize: Theme.fontSize(13)
        }
        OutlineButton {
            width: parent.width
