...
Rectangle {
    ...
    readonly property var protocolIds: ["p25", "dmr", "nxdn48", "nxdn", "nfm"]
    readonly property var protocolLabels: [qsTr("P25"), qsTr("DMR"), qsTr("NXDN48"), qsTr("NXDN96"), qsTr("Analog NFM")]
    ...
    function applyScanProfile(values) {
        draft = Object.assign({}, draft, values);
        timingText = {defaultDwellMs: String(values.defaultDwellMs),
            defaultHoldMs: String(values.defaultHoldMs), maxVisitMs: String(values.maxVisitMs)};
        showValidation("");
    }
    ...
    function openFor(row) {
        ...
        timingText = {
            ...
            maxVisitMs: draft.maxVisitMs !== undefined && Number(draft.maxVisitMs) >= 0 ? String(Number(draft.maxVisitMs)) : ""
        };
        entryInputText = {};
        freqName.text = "";
        frequency.text = "";
    ...
    PlexFlickable {
        ...
        Column {
            ...
            ScanProfiles {
                ...
                onChosen: function(values) { screen.applyScanProfile(values); }
            }

            MicroLabel { text: qsTr("Entries") }
            ...
            UiPanel {
                ...
                Column {
                    ...
                    PlexCheckBox {
                        ...
                        onToggled: screen.setDraft("voiceOnly", checked)
                    }
                    PlexCheckBox {
                        objectName: "scanFinishCalls"
                        width: parent.width
                        text: qsTr("Finish active calls before moving (no visit limit)")
                        checked: Number(screen.draft.maxVisitMs) === 0
                        onToggled: {
                            screen.setDraft("maxVisitMs", checked ? 0 : 5000);
                            screen.timingText = Object.assign({}, screen.timingText, {maxVisitMs: checked ? "0" : "5000"});
                        }
                    }
                    Repeater {
                        id: timingFields
                        model: [
                            {"key": "defaultDwellMs", "label": qsTr("Default dwell"), "minimum": 250, "maximum": 600000},
                            {"key": "defaultHoldMs", "label": qsTr("Default hold"), "minimum": 250, "maximum": 600000},
                            {"key": "maxVisitMs", "label": qsTr("Maximum visit per target"), "minimum": 1000, "maximum": 3600000}
                        ]
                        Column {
                            required property var modelData
                            ...
                            PlexTextField {
                                ...
                                function parsedValue() {
                                    if (!text.length)
                                        return modelData.key === "maxVisitMs" ? -1 : 0;
                                    var value = screen.parseIntStrict(text, 0, modelData.maximum);
                                    return value === 0 || value >= modelData.minimum ? value : NaN;
                                }
                                objectName: "scanTuner_" + modelData.key
                                width: parent.width
                                label: modelData.label
                                unit: qsTr("ms")
                                hint: modelData.key === "maxVisitMs"
                                    ? qsTr("Empty: inherit. 0: no limit. A limit can cut calls short; target overrides still apply.")
                                    : qsTr("Scan default when empty or 0")
                                mono: true
                                text: screen.timingText[modelData.key] || ""
                                error: isNaN(parsedValue()) ? qsTr("Enter 0 or %1–%2 ms.").arg(modelData.minimum).arg(modelData.maximum) : ""
                                inputMethodHints: Qt.ImhDigitsOnly
                                input.validator: RegularExpressionValidator {
                                    regularExpression: /^-?[0-9]*$/
                                }
                                input.onTextEdited: {
                                    ...
                                    screen.setDraft(modelData.key, isNaN(parsedValue()) ? -2 : parsedValue());
                                }
                            }
                        }
                    }
