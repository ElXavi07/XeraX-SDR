...
Window {
    ...
    Loader {
        anchors.fill: safeArea; z: 210
        active: mainRoot.rangeScannerOpen
        sourceComponent: Component { RangeScannerScreen { onClosed: mainRoot.rangeScannerOpen=false } }
    }
    property var captureOriginal: null
    property int captureSeconds: 0
    ReceiverToolsScreen {
        id: quickReceiverTools
        anchors.fill: safeArea; z: 200
        visible: mainRoot.receiverToolsOpen
        enabled: visible
        onClosed: mainRoot.receiverToolsOpen = false
    }
    NavigationLayer { surface: quickReceiverTools; active: mainRoot.receiverToolsOpen; onLeave: mainRoot.receiverToolsOpen = false }
    Connections {
        target: receiverAssistant
        function onSwitchSite(uid) {
            if (decoderHost.sessionState === 4 || (decoderHost.running && metrics.slot1CallState !== 2 && metrics.slot2CallState !== 2 && !metrics.heldTg && !metrics.scanHold))
                mainRoot.restartSite(uid);
        }
        function onRetryRequested() { mainRoot.retrySource(); }
        function onCaptureRequested() {
            if (!decoderHost.running || mainRoot.captureOriginal || mainRoot.sessionScanList || !mainRoot.sessionSystem) return;
            var capture = receiverAssistant.prepareCapture(mainRoot.sessionSystem, metrics.centerFreqHz);
            if (!capture.iqCapturePath) return;
            mainRoot.captureOriginal = Object.assign({}, mainRoot.sessionSystem);
            if (mainRoot.exploring) mainRoot.captureOriginal.freqMhz = Util.mhzText(metrics.centerFreqHz);
            mainRoot.captureSeconds = 0;
            mainRoot.pendingRestart = {kind: "capture", system: capture};
            decoderHost.stop();
        }
        function onFinishCaptureRequested() { mainRoot.finishCapture(); }
    }
    function finishCapture() {
        if (!captureOriginal) return;
        var original = captureOriginal;
        captureOriginal = null;
        pendingRestart = {kind: "mode", system: original, uid: original.uid || "", spectrum: false};
        decoderHost.stop();
    }
    Timer {
        interval: 1000; repeat: true; running: !!mainRoot.captureOriginal
        onTriggered: {
            if (decoderHost.running && mainRoot.sessionSystem && mainRoot.sessionSystem.iqCapturePath) {
                ...
                receiverAssistant.captureFinished();
            }
        }
    }
    Connections {
        target: receiverExpansion
        function onRestartRequested(system) {
            mainRoot.pendingRestart = {kind: "mode", system: system, uid: system.uid || "", spectrum: false};
            decoderHost.stop();
        }
    }
    Connections {
    ...
    onSessionSystemChanged: { receiverAssistant.setSession(sessionSystem ? sessionSystem.uid || "" : "", sessionScanList); receiverExpansion.setSession(sessionSystem || {}); rangeScanner.setSession(sessionSystem || {}); if(sessionSystem && sessionSystem.rangeScanOptions) mainRoot.rangeScannerOpen=true; }
    onSessionScanListChanged: receiverAssistant.setSession(sessionSystem ? sessionSystem.uid || "" : "", sessionScanList)
    ...
    // Analog needs a different front-end output kind, de-emphasis and audio
    // stream. Restart through the existing Idle gate so all three change together.
    function restartListeningMode(flag) {
        if (decoderHost.sessionState !== 2 || !sessionSystem || sessionScanList || metrics.tunerControlled || rangeScanner.active)
            return;
        var sys = Object.assign({}, sessionSystem, {decodeFlag: flag});
        if (metrics.centerFreqHz > 0)
            sys.freqMhz = Util.mhzText(metrics.centerFreqHz);
        if (exploring) prefs.exploreDecodeFlag = flag;
        pendingRestart = {kind: "mode", system: sys, uid: exploring ? "" : (sys.uid || ""), spectrum: spectrumOpen};
        sessionRadio.visible = false;
        decoderHost.stop();
    }
    function restartSite(uid) {
        cancelPendingRestart();
        if ((decoderHost.sessionState !== 2 && decoderHost.sessionState !== 4) || savedSystems.rowForUid(uid) < 0)
            return;
        pendingStart = null;
        pendingStartRow = -1;
    ...
    function finishPendingRestart() {
        ...
        if (request.kind === "capture") {
            startWithMap(request.system, -1, false);
            return;
        }
        if (request.kind === "mode") {
            if (receiverAssistant.status.capturing) receiverAssistant.captureFinished();
            var modeRow = request.uid ? savedSystems.rowForUid(request.uid) : -1;
            if (request.uid && modeRow < 0) {
                startError = qsTr("This source was removed. Choose another source.");
                return;
            }
            startWithMap(request.system, modeRow, false);
            spectrumOpen = request.spectrum && monitorMode;
            return;
        }
    ...
    Component.onCompleted: {
        updateSystemBars();
        if(decoderHost.running && !sessionSystem) sessionSystem=receiverExpansion.restoredSession();
    }
    onActiveChanged: {
        if (active)
            updateSystemBars();
    ...
    function startExploring() {
        ...
        mainRoot.startWithMap(mainRoot.exploreSystem(source, prefs.exploreHost, prefs.explorePort, prefs.exploreFreqMhz, prefs.exploreDecodeFlag), -1, false);
    }
     ...
     * monitor header and the args builder share the ordinary start path. The
     * selected analog/digital mode is explicit; trunking stays off so manual
     * tuning remains available.
     */
    function exploreSystem(source, host, port, freqMhz, decodeFlag) {
        return {
            name: qsTr("Exploring"),
            sourceType: source,
            host: host,
    ...
    Connections {
        target: receiverExpansion
        function onSelectSite(uid) {
            if(decoderHost.sessionState===2) mainRoot.restartSite(uid);
            else if(decoderHost.sessionState===0 || decoderHost.sessionState===4) {
                var row=savedSystems.rowForUid(uid); if(row>=0) mainRoot.startSystem(row);
            }
        }
        function onCaptureRestored() {
            if(mainRoot.captureOriginal) { mainRoot.sessionSystem=mainRoot.captureOriginal; mainRoot.captureOriginal=null; }
            receiverAssistant.captureFinished();
        }
    }
    // ---- Tab shell ----
    ...
    ExploreSetupScreen {
        ...
        onStartRange: function(sourceType, host, port, settings) {
            prefs.exploreSourceType=sourceType; prefs.exploreHost=host; prefs.explorePort=port;
            prefs.exploreFreqMhz=settings.firstMhz; prefs.exploreDecodeFlag=settings.decodeFlag;
            rangeScanner.saveSettings(settings);
            var system=mainRoot.exploreSystem(sourceType,host,port,settings.firstMhz,settings.decodeFlag);
            system.name=qsTr("Frequency range scanner"); system.bandwidthKhz=48; system.rangeScanOptions=settings;
            mainRoot.exploreSetupOpen=false;
            mainRoot.startWithMap(system,-1,false);
        }
        onStart: function (sourceType, host, port, freqMhz, decodeFlag) {
            ...
            prefs.exploreDecodeFlag = decodeFlag;
            mainRoot.exploreSetupOpen = false;
            mainRoot.startWithMap(mainRoot.exploreSystem(sourceType, host, port, freqMhz, decodeFlag), -1, false);
        }
    }
    ...
    MonitorScreen {
        onOpenAudioCheck: sessionRadio.open()
        onOpenReceiverTools: { if(rangeScanner.active) mainRoot.rangeScannerOpen=true; else mainRoot.receiverToolsOpen=true; }
        id: monitor
    ...
    Component {
        ...
        SpectrumScreen {
            ...
            onExploreFromHere: {
                if(rangeScanner.active) rangeScanner.stop();
                ...
                mainRoot.sessionSystem = mainRoot.exploreSystem(mainRoot.sessionSystem ? mainRoot.sessionSystem.sourceType : "usb", mainRoot.sessionSystem ? mainRoot.sessionSystem.host : "", mainRoot.sessionSystem ? mainRoot.sessionSystem.port : 0, exploreFreqMhz, metrics.decodeMode === commands.decodeModeForFlag("-fA") ? "-fA" : (mainRoot.sessionSystem ? mainRoot.sessionSystem.decodeFlag : "-fa"));
    ...
    Item {
        ...
        Item {
            ...
            Text {
                ...
                anchors.right: parent.right
                anchors.verticalCenter: parent.verticalCenter
                text: mainRoot.sessionDestination === "history" ? qsTr("Calls") : qsTr("Settings")
                font.family: Theme.sans
    ...
    ActionMenu {
        ...
        actions: {
            ...
            items.push({text: qsTr("Scan"), key: "scan", objectName: "sessionMenuScan"});
            items.push({text: qsTr("Tools"), key: "tools", objectName: "sessionMenuTools"});
            items.push({text: qsTr("Talkgroups"), key: "talkgroups", objectName: "sessionMenuTalkgroups"});
            items.push({text: qsTr("Calls"), key: "history", objectName: "sessionMenuHistory"});
        ...
        onTriggered: function (index) {
            ...
            if (key === "sites")
                ...
                mainRoot.rangeScannerOpen=true;
            } else if (key === "tools") {
                mainRoot.receiverToolsOpen=true;
            }
    ...
    RadioSheet {
        id: sessionRadio
        anchors.fill: safeArea
        canRestartMode: !!mainRoot.sessionSystem && !mainRoot.sessionScanList && !metrics.tunerControlled && !rangeScanner.active
        onRestartModeRequested: function(flag) { mainRoot.restartListeningMode(flag); }
    }
