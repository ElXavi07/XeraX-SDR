...
/** Shared scan timing validation for editor validation and session start. */
QString session_args_scan_timing_error(const QVariantMap& list);
...
class SessionArgsBuilder : public QObject {
  ...
  public:
    ...
    // Private C++ transport to another app-owned process; never exposed through QML.
    QStringList workerArgs(const QVariantMap& system, SessionArgsError* error) const { return buildArgs(system,error); }
