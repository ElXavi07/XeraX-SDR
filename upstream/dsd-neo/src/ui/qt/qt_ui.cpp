...
ui_load(QQmlApplicationEngine& engine, DecoderHost* host) {
    ...
    auto* language = new AppLanguage(&engine);
    ...
    auto* receiverTools = new ReceiverTools(prefs, systems, scanLists, host, &engine);
    context->setContextProperty(QStringLiteral("receiverTools"), receiverTools);
    auto* assistant = new ReceiverAssistant(&engine);
    assistant->configure(metrics, commands, host, receiverTools, systems);
    context->setContextProperty("receiverAssistant", assistant);
    auto* expansion = new ReceiverExpansion(&engine);
    expansion->configure(metrics,commands,host,systems,sessionArgs);
    context->setContextProperty("receiverExpansion",expansion);
    auto* aiReceiver = new AiReceiver(&engine);
    aiReceiver->configure(metrics,host,receiverTools,assistant,expansion,language);
    context->setContextProperty("aiReceiver",aiReceiver);
    auto* rangeScanner = new RangeScanner(&engine);
    rangeScanner->configure(metrics,commands,host,spectrum,receiverTools,assistant,expansion,systems);
    context->setContextProperty("rangeScanner",rangeScanner);
    context->setContextProperty("callLibrary",new CallLibrary(&engine));
    QObject::connect(receiverTools, &ReceiverTools::changed, spectrum, [receiverTools, spectrum] {
        const auto health = receiverTools->health();
        spectrum->setEconomyMode(health.value("powerSave").toBool() || health.value("thermal").toInt() >= 2);
    });
