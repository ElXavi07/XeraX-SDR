...
CommandBridge::tuneHz(unsigned int hz) const {
    if(m_rangeScanOwned) return false;
...
bool
// cppcheck-suppress functionStatic -- Q_INVOKABLE members cannot be static (Qt meta-object)
CommandBridge::setAutoPpm(bool enabled) const {
    return accepted(dsd_app_command_set_i32(DSD_APP_CMD_RTL_SET_AUTO_PPM, enabled ? 1 : 0));
}

bool
CommandBridge::setPpm(int ppm) const {
    if (ppm < -200 || ppm > 200 || !setAutoPpm(false)) return false;
...
bool CommandBridge::rangeTuneHz(unsigned int hz) const {
    return m_rangeScanOwned && accepted(dsd_app_command_set_u32(DSD_APP_CMD_MANUAL_TUNE,hz));
}
bool CommandBridge::rangeDecodeMode(int mode) const {
    return m_rangeScanOwned && accepted(dsd_app_command_set_i32(DSD_APP_CMD_DECODE_MODE_SET,mode));
}
