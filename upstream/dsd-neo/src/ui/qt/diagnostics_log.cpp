...
DiagnosticsLog::instance() {
    ...
    static auto* log = new DiagnosticsLog((qEnvironmentVariableIsSet("XERAX_PROCESS_DATA_DIR")
        ? qEnvironmentVariable("XERAX_PROCESS_DATA_DIR") : QStandardPaths::writableLocation(QStandardPaths::AppDataLocation))
...
void
DiagnosticsLogModel::copyAll(const QString& summary) const {
    if (auto* clipboard = QGuiApplication::clipboard()) {
        clipboard->setText(summary + allText());
    }
}
