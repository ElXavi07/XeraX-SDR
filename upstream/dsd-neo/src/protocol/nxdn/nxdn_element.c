...
nxdn_vcall_load_key(const dsd_opts* opts, dsd_state* state, const struct nxdn_vcall_info* info) {
    ...
    if (info->cipher_type == 1U) {
        // NXDN48 can signal a scrambler key ID; destination presets also work for
        // NXDN96. A miss must not reuse the previous call's material.
        const int old_present = dsd_key_scalar_present(state, 0);
        const unsigned long long old_key = state->R;
        int selected = -1;
        if (dsd_frame_sync_active_nxdn_variant(opts, state) == DSD_NXDN_VARIANT_48
            && keyring_scalar_present(state, info->key_id)) {
            selected = info->key_id;
        } else if (keyring_scalar_present(state, info->destination_id)) {
            selected = info->destination_id;
        }
        keyring_activate_slot_with_kid(state, 0, selected);
        if (old_present != dsd_key_scalar_present(state, 0) || old_key != state->R) {
            state->payload_miN = 0;
        }
    } else if (info->cipher_type == 2U) {
        keyring_activate_slot_with_kid(state, 0, info->key_id);
    ...
    if (dsd_call_state_update_crypto(state, 0U, &update) > 0) {
        ...
        if (dsd_call_state_get(state, 0, &call) > 0) {
            ...
            if (state->keyloader == 1) {
                ...
                if ((cipher_type != 1 || dsd_frame_sync_active_nxdn_variant(opts, state) == DSD_NXDN_VARIANT_48)
                    && keyring_scalar_present(state, key_id)) {
                    source = DSD_CALL_KEY_SIGNALED;
                    selected = key_id;
                } else if (cipher_type == 1
                           && call.ota_target_id < sizeof(state->rkey_array) / sizeof(state->rkey_array[0])
                           && keyring_scalar_present(state, (int)call.ota_target_id)) {
