...
dmr_dburst_handle_bptc(dmr_data_burst_ctx* ctx) {
    ...
    if ((ctx->opts->aggressive_framesync == 0 || dsd_dmr_ras_enabled()) && ctx->crc_correct == 0 && ctx->irrecoverable_errors == 0
...
dmr_dburst_finalize_status(dmr_data_burst_ctx* ctx) {
    // Count independently checked control headers and embedded link control.
    // Continuation/unconfirmed payload blocks have no standalone CRC verdict.
    if((ctx->is_bptc && ctx->crclen >= 16) || ctx->is_emb)
        dsd_dmr_evidence_note(ctx->slot,ctx->is_ras,ctx->irrecoverable_errors,ctx->crc_correct);
    if (ctx->is_ras == 1) {
