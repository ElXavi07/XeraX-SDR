...
handle_grant(p25_sm_ctx_t* ctx, dsd_opts* opts, dsd_state* state, const p25_sm_event_t* ev) {
    ...
    // The policy and band-plan gates above still apply. The control receiver stays
    // parked and leaves call state to the separate voice process.
    if (dsd_dual_offer((uint32_t)grant.freq, (uint32_t)state->p2_wacn, (uint32_t)state->p2_sysid,
        (uint32_t)state->p2_cc, (uint32_t)grant.target_id, (uint32_t)ev->src,
        is_tdma_channel(state, ev->channel), grant.slot, decision.priority)) return;
