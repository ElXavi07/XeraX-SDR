...
dsd_engine_prepare_scan_profile(const dsd_opts* opts, dsd_state* state, long int freq, int ted_sps) {
    if (opts->audio_in_type == AUDIO_IN_RTL && ted_sps > 0) {
        if (opts->analog_only) {
            dsd_engine_prepare_retune_profile_for_target(opts, state, (uint32_t)freq, 0, 4800, 2,
                                                         RTL_STREAM_CHANNEL_PROFILE_WIDE, 10, 0);
        } else if (dsd_engine_conventional_scan_active(opts)) {
