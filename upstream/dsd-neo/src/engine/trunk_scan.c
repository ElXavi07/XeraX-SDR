...
typedef struct {
    ...
    size_t count;
    size_t active;
    size_t normal_cursor;
    size_t priority_cursor;
...
scan_parse_type(const char* s, dsd_trunk_scan_target_type* out) {
    ...
    if (strcmp(s, "nfm-conventional") == 0) {
        *out = DSD_TRUNK_SCAN_TARGET_NFM_CONVENTIONAL;
        return 0;
    }
...
scan_parse_target_row(char* line, dsd_trunk_scan_target_list* parsed, const dsd_trunk_scan_row_parse* parse) {
    ...
    const char* priority_s = scan_optional_field(fields, field_count, parse->priority_idx);
    if (*priority_s && strcmp(priority_s, "0") && strcmp(priority_s, "1")) {
        scan_set_error(parse->err, parse->err_sz, "row %u priority must be 0 or 1", parse->row);
        return -1;
    }
    target.priority = strcmp(priority_s, "1") == 0;
    const char* chan_csv = "";
    const char* dwell_s = "";
    const char* hold_s = "";
...
trunk_scan_retune_active(dsd_opts* opts, dsd_state* state, dsd_trunk_scan_target_runtime* rt,
    ...
    if (trunk_scan_type_is_conventional(rt->target.type)) {
        const int ted_sps = rt->target.type == DSD_TRUNK_SCAN_TARGET_NFM_CONVENTIONAL ? 10 : trunk_scan_target_is_p25(&rt->target) ? trunk_scan_p25_cc_sps(opts, state)
...
trunk_scan_advance(dsd_opts* opts, dsd_state* state, dsd_trunk_scan_coord* coord) {
    ...
    if (coord->targets[original_active].target.priority) coord->priority_cursor = original_active;
    else coord->normal_cursor = original_active;
    size_t preferred = (original_active + 1U) % coord->count;
    uint8_t* flags = (uint8_t*)calloc(coord->count, 1);
    if (flags) {
        for (size_t i = 0; i < coord->count; ++i)
            flags[i] = ((!coord->targets[i].avoided && coord->targets[i].retry_until_m <= now_m) ? 1 : 0)
                | (coord->targets[i].target.priority ? 2 : 0);
        preferred = dsd_scan_priority_choose(flags, coord->count, original_active,
                                             coord->normal_cursor, coord->priority_cursor);
        free(flags);
    }
    for (size_t attempts = 0; attempts < coord->count; attempts++) {
        size_t next = (preferred + attempts) % coord->count;
...
trunk_scan_tick_targets_locked(dsd_opts* opts, dsd_state* state, dsd_trunk_scan_coord* coord, double now_m) {
    ...
    if (rt->target.type == DSD_TRUNK_SCAN_TARGET_NFM_CONVENTIONAL
        && state->analog_activity_m > rt->visit_since_m + 0.15
        && now_m >= state->analog_activity_m && now_m - state->analog_activity_m < 0.25) {
        rt->last_allowed_activity_m = state->analog_activity_m;
    }
