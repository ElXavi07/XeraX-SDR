...
static const struct ui_cmd_payload_min_size_rule k_ui_cmd_payload_min_size_rules[] = {
    ...
    {DSD_APP_CMD_P25_VOICE_TUNE, sizeof(dsd_voice_tune)},
...
decode_mode_set_early_verdict(const dsd_opts* opts, dsd_state* state, const struct dsd_app_command* c,
    ...
    if (requested < 0 || requested > (int32_t)DSDCFG_MODE_WFM) {
...
rr_apply_preflight(const dsd_opts* opts, dsd_state* state, const dsd_app_rr_apply_payload* p) {
    ...
    /* <=, not <: DSDCFG_MODE_UNSET is 0 and is not a preset, so letting it through
       ...
       true. */
    if (p->decode_mode <= (int32_t)DSDCFG_MODE_UNSET || p->decode_mode > (int32_t)DSDCFG_MODE_WFM) {
...
apply_cmd_unscoped(dsd_opts* opts, dsd_state* state, const struct dsd_app_command* c) {
    ...
    if (c->id == DSD_APP_CMD_P25_VOICE_TUNE) {
        if (!state || c->n != sizeof(dsd_voice_tune)) return UI_CMD_APPLY_INVALID_PAYLOAD;
        dsd_voice_tune tune;
        DSD_MEMCPY(&tune, c->data, sizeof tune);
        if (!dsd_voice_tune_valid(&tune) || opts->trunk_enable || opts->trunk_scan_enabled || opts->scanner_mode
            || opts->audio_in_type != AUDIO_IN_RTL || (!opts->frame_p25p1 && !opts->frame_p25p2)) {
            dsd_voice_tune_result(tune.grant.sequence, -1);
            return UI_CMD_APPLY_FAILED;
        }
        // Drain on the decoder thread: no partial mode/network update can be observed
        // by a frame parser. Retuning flushes the radio pipeline; no USB reopen.
        const int result = decode_mode_apply_value(opts, state, tune.grant.tdma ? DSDCFG_MODE_P25P2 : DSDCFG_MODE_P25P1);
        if (result != UI_CMD_APPLY_COMPLETED) { dsd_voice_tune_result(tune.grant.sequence, -1); return result; }
        struct dsd_app_command mod = {0};
        const int32_t modulation = tune.grant.tdma ? 1 : tune.modulation;
        mod.id = DSD_APP_CMD_MOD_SET; mod.n = sizeof(modulation);
        DSD_MEMCPY(mod.data, &modulation, sizeof(modulation));
        if (ui_cmd_dispatch(opts, state, &mod) != UI_CMD_APPLY_COMPLETED) {
            dsd_voice_tune_result(tune.grant.sequence, -1); return UI_CMD_APPLY_FAILED;
        }
        svc_set_p2_params(state, tune.grant.wacn, tune.grant.sysid, tune.grant.nac);
        svc_set_slot_pref(opts, tune.grant.tdma ? tune.grant.slot : 2);
        svc_set_slots_onoff(opts, tune.grant.tdma ? (1 << tune.grant.slot) : 3);
        const int tuned = svc_rtl_set_freq(opts, state, tune.grant.frequency);
        dsd_voice_tune_result(tune.grant.sequence, tuned == 0 ? 1 : -1);
        return tuned == 0 ? UI_CMD_APPLY_COMPLETED : UI_CMD_APPLY_FAILED;
    }
