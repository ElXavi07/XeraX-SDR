...
/* Conventional frequency browsing. Callbacks own the list and its items. */
typedef struct {
    int id, encrypted;
    long long frequency_hz;
    double lat, lon;
    char name[256], description[256], mode[32], tone[64], color_code[16], talkgroup[32], slot[16], tags[256];
} dsd_rr_conventional_entry;
typedef struct { dsd_rr_conventional_entry* items; size_t count; } dsd_rr_conventional_list;
typedef enum {
    DSD_RR_CONV_COUNTY_CATEGORIES, DSD_RR_CONV_COUNTY_AGENCIES,
    DSD_RR_CONV_AGENCY_CATEGORIES, DSD_RR_CONV_FREQUENCIES, DSD_RR_CONV_MODES
} dsd_rr_conventional_kind;
uint64_t dsd_rr_fetch_conventional(dsd_rr_client*, const dsd_rr_auth*, dsd_rr_conventional_kind, int,
                                   dsd_rr_done_cb, void*);
