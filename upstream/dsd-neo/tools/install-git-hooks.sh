...
echo "Tip: checks run in concurrent lanes sized to the machine; DSD_HOOK_JOBS=N sets the worker budget shared across the run (each check keeps at least one worker), DSD_HOOK_SERIAL=1 runs them one at a time."
