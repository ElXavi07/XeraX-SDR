...
int main(int argc,char** argv) {
    ...
    app.setApplicationVersion("4.3.1-windows.1");
    ...
    engine.rootContext()->setContextProperty("appVersionText",QString("4.3.1 — Windows preview 1"));
    app.setWindowIcon(QIcon(":/dsdneo/qml/xerax-icon.svg"));
    auto* window=qobject_cast<QQuickWindow*>(engine.rootObjects().first());
    if(window) { window->resize(1240,840); window->setMinimumSize(QSize(420,620)); window->setTitle("XeraX SDR 4.3.1 — Windows preview 1"); }
