...
main(void) {
    {
        dsd_opts* o = calloc(1, sizeof(*o));
        dsd_state* s = calloc(1, sizeof(*s));
        assert(o && s);
        assert(dsd_apply_decode_mode_preset(DSDCFG_MODE_P25P2, DSD_DECODE_PRESET_PROFILE_CLI, o, s) == 0);
        o->mod_cli_lock = 1;
        o->pulse_digi_rate_out = 8000;
        o->pulse_digi_out_channels = 2;
        assert(dsd_scan_mode_enter(o, s, DSD_SCAN_MODE_NFM) == 0);
        assert(o->analog_only && o->monitor_input_audio && !o->frame_p25p2 && !o->frame_dmr);
        assert(!o->mod_qpsk && s->rf_mod == 0);
        assert(o->pulse_digi_out_channels == 2 && o->pulse_digi_rate_out == 8000);
        assert(dsd_scan_mode_enter(o, s, DSD_SCAN_MODE_DMR) == 0);
        assert(!o->analog_only && !o->monitor_input_audio && o->frame_dmr);
        assert(dsd_scan_mode_enter(o, s, DSD_SCAN_MODE_NFM) == 0);
        dsd_scan_mode_leave(o, s);
        assert(!o->analog_only && o->frame_p25p2);
        free(s);
        free(o);
    }
