...
create_temp_file_with_suffix(const char* prefix, const char* suffix, char* out_path, size_t out_path_sz) {
    ...
    // Windows cannot rename the open CRT descriptor returned by mkstemp.
    // Close before renaming, then return a descriptor for the same test file.
    (void)dsd_close(fd);
    ...
    FILE* reopened = fopen(out_path, "r+b");
    if (!reopened) {
        (void)remove(out_path);
        return -1;
    }
    fd = dsd_dup(dsd_fileno(reopened));
    fclose(reopened);
    return fd;
}
...
test_manual_and_float_autogain_helpers(void) {
    ...
    rc |= expect_float_close("analog gain rtl positive", rtl_samples[0], 8191.75f, 1e-3f);
    rc |= expect_float_close("analog gain rtl negative", rtl_samples[1], -16383.5f, 2e-3f);
