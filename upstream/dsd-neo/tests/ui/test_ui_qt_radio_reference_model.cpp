...
test_trunked_system(void) {
    ...
    const double indioMiles = firstSite.value(QStringLiteral("indioDistanceMi")).toDouble();
    expect("Iowa site is far outside the Indio preset", indioMiles > 1400 && indioMiles < 1700);
    expect("missing coordinates have no Indio distance",
           h.model.sites().last().toMap().value(QStringLiteral("indioDistanceMi")).toDouble() == -1);
...
void
test_indio_site_distance(void) {
    Harness h;
    h.fake.replaceFrom = "<lat xsi:type=\"xsd:decimal\">41.65503</lat>"
                         "<lon xsi:type=\"xsd:decimal\">-91.60244</lon>";
    h.fake.replaceTo = "<lat xsi:type=\"xsd:decimal\">33.7206</lat>"
                       "<lon xsi:type=\"xsd:decimal\">-116.2156</lon>";
    h.model.loadSystem(6673);
    pump(h.model);
    expect("Indio replacement fixture loaded", h.model.sites().size() == 35);
    if (!h.model.sites().isEmpty()) {
        const QVariant distance = h.model.sites().first().toMap().value(QStringLiteral("indioDistanceMi"));
        // The decimal SOAP parser can round at a different last bit than a
        // compiled double literal; sub-millimeter error is immaterial here.
        expect("Indio center has zero miles", distance.isValid() && distance.toDouble() >= 0
                                                  && distance.toDouble() < 0.000001);
    }
}
