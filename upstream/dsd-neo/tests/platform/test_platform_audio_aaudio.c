...
static struct {
    ...
    int32_t granted_device_id;
    int device_selection_calls;
    int32_t device_rate;
...
reset_fakes(void) {
    ...
    (void)dsd_audio_set_output_device_id(0);
    g.device_rate = 8000;
    g.device_channels = 2;
...
void
AAudioStreamBuilder_setDeviceId(AAudioStreamBuilder* builder, int32_t device_id) {
    (void)builder;
    g.requested_device_id = device_id;
    g.device_selection_calls++;
}
...
AAudioStream_write(AAudioStream* stream, const void* buffer, int32_t frames, int64_t timeout_ns) {
    ...
    (void)timeout_ns;
    g.write_calls++;
    if (g.cancel_test_after && g.write_calls == g.cancel_test_after) dsd_audio_cancel_output_test();
...
static void
test_output_device_selection(void) {
    reset_fakes();
    dsd_audio_params params = valid_params(8000, 2);
    assert(dsd_audio_set_output_device_id(17) == 0);
    dsd_audio_stream* stream = dsd_audio_open_output(&params);
    assert(stream != NULL);
    assert(g.requested_device_id == 17);
    const int opens = g.open_calls;
    const int closes = g.close_calls;
    assert(dsd_audio_set_output_device_id(29) == 0);
    assert(g.open_calls == opens && g.close_calls == closes); /* UI never closes a stream. */
    assert(dsd_audio_write(stream, (const int16_t[]){1, 2, 3, 4}, 2) == 2);
    assert(g.requested_device_id == 29);
    assert(g.open_calls == opens + 1 && g.close_calls == closes + 1);
    assert(dsd_audio_output_device_id() == 29);
    (void)dsd_audio_write(stream, (const int16_t[]){1, 2}, 1);
    assert(g.open_calls == opens + 1); /* no reopen without a new request */
    (void)dsd_audio_set_output_device_id(29);
    (void)dsd_audio_write(stream, (const int16_t[]){1, 2}, 1);
    assert(g.open_calls == opens + 2); /* explicitly selecting it again retries the route */
    assert(dsd_audio_set_output_device_id(-1) == -1);
    assert(atomic_load(&s_requested_output_device_id) == 29);
    (void)dsd_audio_set_output_device_id(0);
    (void)dsd_audio_write(stream, (const int16_t[]){1, 2}, 1);
    assert(g.requested_device_id == 0);
    assert(dsd_audio_output_device_id() == 42); /* actual default, not requested 0 */
    dsd_audio_close(stream);
}

static void
test_output_route_fallback_and_failure(void) {
    reset_fakes();
    g.reject_rate = 8000;
    g.device_rate = 48000;
    (void)dsd_audio_set_output_device_id(17);
    dsd_audio_params params = valid_params(8000, 2);
    dsd_audio_stream* stream = dsd_audio_open_output(&params);
    assert(stream != NULL && g.open_calls >= 2 && g.requested_device_id == 17);
    g.granted_device_id = 99;
    (void)dsd_audio_write(stream, (const int16_t[]){1, 2}, 1);
    assert(dsd_audio_output_device_id() == 99); /* OS can decline the requested route. */
    (void)dsd_audio_set_output_device_id(23);
    g.open_result = AAUDIO_ERROR_UNAVAILABLE;
    (void)dsd_audio_write(stream, (const int16_t[]){1, 2}, 1);
    (void)dsd_audio_write(stream, (const int16_t[]){1, 2}, 1);
    int failed_opens = g.open_calls;
    (void)dsd_audio_write(stream, (const int16_t[]){1, 2}, 1);
    assert(g.open_calls == failed_opens); /* no reopen storm */
    g.open_result = AAUDIO_OK;
    (void)dsd_audio_set_output_device_id(17); /* a new choice retries immediately */
    (void)dsd_audio_write(stream, (const int16_t[]){1, 2}, 1);
    assert(stream->handle != NULL && g.requested_device_id == 17);
    dsd_audio_close(stream);

    reset_fakes();
    (void)dsd_audio_set_output_device_id(17);
    stream = dsd_audio_open_input(&params);
    assert(stream != NULL && g.device_selection_calls == 0);
    (void)dsd_audio_set_output_device_id(29);
    int16_t input[2];
    (void)dsd_audio_read(stream, input, 1);
    assert(g.device_selection_calls == 0); /* never redirect a microphone */
    dsd_audio_close(stream);
}

static void test_real_audio_diagnostics(void) {
    reset_fakes();
    (void)dsd_audio_set_output_device_id(17);
    dsd_audio_live_suppress(1);
    uint64_t received = dsd_audio_received_frames(), output = dsd_audio_output_frames();
    assert(dsd_audio_test_output(dsd_audio_output_test_token()) == 0);
    assert(g.write_calls >= 60 && g.captured_samples > 0 && g.requested_device_id == 17);
    int nonzero = 0;
    for (size_t i=0;i<FAKE_CAPTURE_SAMPLES;++i) {
        assert(g.captured[i] >= -2600 && g.captured[i] <= 2600);
        nonzero += g.captured[i] != 0;
    }
    assert(nonzero > 100 && dsd_audio_live_suppressed());
    assert(dsd_audio_received_frames() == received && dsd_audio_output_frames() == output);
    assert(atomic_load(&s_output_stream_count) == 0);
    reset_fakes(); g.reject_rate = 8000; g.device_rate = 48000;
    assert(dsd_audio_test_output(dsd_audio_output_test_token()) == 0);
    reset_fakes(); g.open_result = AAUDIO_ERROR_UNAVAILABLE;
    assert(dsd_audio_test_output(dsd_audio_output_test_token()) == -1);
    reset_fakes(); int token = dsd_audio_output_test_token(); dsd_audio_cancel_output_test();
    assert(dsd_audio_test_output(token) == -2 && g.open_calls == 0);
    reset_fakes(); g.cancel_test_after = 2;
    assert(dsd_audio_test_output(dsd_audio_output_test_token()) == -2);
    assert(atomic_load(&s_output_stream_count) == 0 && g.write_calls == 2);
    reset_fakes(); g.write_result_pending = 1; g.write_result = AAUDIO_ERROR_DISCONNECTED;
    assert(dsd_audio_test_output(dsd_audio_output_test_token()) == -1);
    ...
    dsd_audio_stream* stream = dsd_audio_open_output(&params);
    assert(stream);
    const int16_t pcm[4] = {100,200,-100,-200};
    output = dsd_audio_output_frames();
    assert(dsd_audio_write(stream, pcm, 2) == 2);
    assert(dsd_audio_output_frames() == output + 12);
    dsd_audio_live_suppress(1);
    assert(dsd_audio_write(stream, pcm, 2) == 2);
    assert(dsd_audio_output_frames() == output + 12);
    dsd_audio_live_suppress(0);
...
int
main(void) {
    test_real_audio_diagnostics();
    test_output_device_selection();
    test_output_route_fallback_and_failure();
...
AAudioStream_getDeviceId(AAudioStream* stream) {
    (void)stream;
    return g.granted_device_id > 0 ? g.granted_device_id : (g.requested_device_id > 0 ? g.requested_device_id : 42);
}
